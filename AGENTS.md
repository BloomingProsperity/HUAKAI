...
1. `AGENTS.md` 与 `docs/RULES.md` 保存当前执行规则；
2. `docs/HUAKAI项目与架构白皮书.md` 保存长期产品、架构、模块、链路和路线真相；
3. `docs/源码责任索引.md` 由当前源码生成，保存第一方文件职责与影响半径；
4. `docs/runbooks/` 与 `docs/deploy/` 只保存当前可执行的运维手册；
5. OpenAPI、schema、生成清单和测试保存机器可验证合同；
6. Issue、PR、commit、CI 和 Release 保存过程与历史证据。

完成的计划、审查报告、研究稿、会话交接、重复总纲和过期运维稿不得长期留在主线充当第二套真相；有效结论吸收到上述现行入口后删除。Git 历史负责追溯，不在仓库继续堆“备份文档”。

开始工作前至少读取：

1. `docs/RULES.md`；
2. 当前唯一 GitHub Issue 中的执行计划；
...
- 所有调查结论、代码修改、测试和提交都以最新 `origin/main` 为基线。开始工作和提交 PR 前必须先 `git fetch origin main`，核对当前功能分支已包含最新主线；有主线增量时，先把增量无丢失地同步到当前功能分支并处理冲突，再继续实现或提交。
- 白皮书、架构图、源码责任索引和“当前已实现”结论只能从最新 `origin/main` 提取。其他工作树、未合并分支和未合并 PR 只代表待审变更；除非已经进入主线，否则不得作为当前项目能力或文档补丁来源。
- 禁止直接在 `main` 分支或主线工作树修改、提交、推送业务内容；主线只接受经过验证并获 Owner 批准的 PR。所有改动必须在当前唯一功能分支完成，不得为了同步主线另开分支或 worktree。
- 同一目标只保留一个 GitHub Issue 作为当前计划与状态源、一个干净功能分支和一个 PR；不得为同一问题继续开分支、Issue 或计划文档。
- 未经 Owner 同意不得合并主线；不得触碰另一个目标或未授权 worktree。
- 大型临时文件、构建缓存和抓取产物不得放 `/tmp`，使用当前磁盘上的项目缓存目录。
- 当前工作未完成实现、测试、复审和记录时，Owner 中途新增需求只进入当前工作之后的队列；只有 Owner 明确要求暂停、替换或调整优先级时才切换，不允许一套未完成就并行开第二套。
- 已经独立验证、CI 全绿并获 Owner 明确批准的闭环切片可以先合主线；该次合并只包含已验证提交，未验证工作必须留在工作树。前一 PR 合并后，剩余目标只允许再开一个活动 PR，禁止为了整理方便拆出多条并行 PR。
- 分支只承担正在进行的工作。PR 合并、关闭或功能被主线等价吸收后，核实无独有提交并删除对应本地与远端分支；不得长期保留 `backup/`、`archive/`、旧实验分支或已完成目标分支干扰判断。

项目级运行合同：

- 身份只有三类：部署者管理身份 `platform_admin`、下级租户管理身份 `tenant_operator`、最终用户 `users.role='user'`。下级租户不是额外的“代理”身份；最终用户可以属于部署者工作租户或下级租户。部署者可以调整下级租户钱包，不得越级调整该租户最终用户；租户管理员只能管理自身租户和本租户最终用户。权限与目标租户必须来自认证上下文和数据库事实，不接受请求体自报身份。
- 统一 Dispatcher 不等于统一 TLS 实现。官方 API Key 使用 Go standard transport；只有经认证模式合同明确要求客户端指纹的会话/OAuth 账号才进入 Rust mimicry。Rust 是 mimicry 唯一实现，mimicry 不得静默回退 standard，官方 Key 也不得误入 mimicry。该矩阵对业务转发、模型发现、额度探测和账号测试共同生效。
- 分组路由策略、能力授权或租户作用域的权威配置缺失、读取失败或无法判定时必须 fail-closed，返回稳定且可辨识的错误；禁止临时放行、沿用不确定授权或进入本租户更高等级账号池。
...
```text
=== CLEAN-ROOM LANE GUARD (HUAKAI CR-001..CR-006) ===
...
非平凡工作必须更新当前唯一 GitHub Issue，而不是创建 Claude/Codex 双计划、会话交接稿或多份
补充报告。Issue 正文或置顶状态评论中的计划最少包含：
...
旧计划被新计划覆盖时删除或合并，只保留同一 Issue 中的最新权威版本。单字符修复、单个既有测试和纯只读核实可不改计划。

计划必须在实施前向 Owner 简要展示范围、顺序、风险和成功标准；Owner 已授权当前目标时，展示不等于重新等待批准，除非命中 §3.1 决策停门。

当前计划是临时执行物，不是白皮书，也不是永久操作记录。目标完成后把长期结论写入白皮书或
现行手册，把缺陷与变更证据留在 Issue/PR/commit/Release，关闭 Issue，不再把计划复制成
Markdown 归档。
...
- 一个源文件对应一个内聚职责；新增功能不得继续塞进已超预算文件，优先落到职责清晰的新包或子包。
- 一个 Go 包目录默认不超过 6000 非测试行或 20 个非测试文件；存量超标项不做无需求的专项重构，修复时只做最小必要改动，并受 `backend/internal/codebudget/baseline.json` 的既有 +5% 余量约束。
- 禁止为了过门抬高 baseline；只有有意拆分使体量下降后才能重生成。
- 不把顺手拆大文件混进普通 Bug PR；只有新能力需要、关键缺陷无法安全修复或 Owner 单独立项时，才做有判别测试保护的局部拆分。
...
- 当前主线不保留旧业务前端。新前端必须以生产源码、OpenAPI、三身份权限和运行合同为依据独立建设，
...
reviewer 必须逐项覆盖 acceptance ID，引用行为合同与测试 `file:line`，检查弱断言、假 fixture、缺失失败门和运维恢复。REJECT 时不得宣称完成。
...
- Owner 明确批准先合并已验证闭环切片时，只合并已推送且检查通过的提交；工作树里的未验证 diff 不得夹带。已合并 PR 关闭后，剩余目标只保留一个新的活动 PR。
- 提交前检查 dirty tree，明确哪些是当前改动、哪些是用户或其他目标改动；不回滚他人内容。
- Owner 当前关闭并行双计划；主线现存 `.coordination/` 工具不得在单目标工作中自动启用，也不能据此复活旧的平行计划合同。未来若恢复并行，必须由 Owner 重新确认当时的协调规则。
- PR 合并或关闭后，先确认提交已进入主线或已被明确取代，再删除对应本地与远端分支。活动 PR、尚有独有提交或 Owner 明确保留的分支不得删除。
- 不永久指定某个 AI 的职责。发生执行者交接时，原执行者在当前 Issue 或 PR 记录分支、最后可信
  SHA、已完成、未完成、验证、阻塞、工作树状态和外部副作用；接手者重新 fetch 并核对 Issue、
  PR、diff 与最小相关验证后才能继续。不得新建交接 Markdown，也不得把共用 GitHub 账号下的
  评论冒充平台可证明的独立人工审批。
