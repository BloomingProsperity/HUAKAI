...
// ── 能力绑定 upsert (PUT /v1/admin/models/{id}/capability-bindings) ───────
// 后端 controlhttp/model_admin_aliases_handler.go capabilityBindingUpsertRequest:
//   {scope, capability, capability_value?, capability_params?, enabled, tenant_id?}; **无 source**(服务端强制 operator);
//   enabled 用 *bool 必填(省略→400, 防静默翻 disabled); DisallowUnknownFields 拒未知/拒 body source。
//   能力名合法性由后端 store 权威校验(ErrInvalidModelCapability→400 invalid_capability), 前端只 fail-fast 非空。
...
// validateCapabilityBinding 镜像后端 upsertModelCapabilityBinding 的形态前置校验(发请求前预拒明显错误)。
// scope∈{tenant,global}; tenant scope 需 tenant_id>0; capability 非空。返回 null=合法; 否则错误文案。
export function validateCapabilityBinding(input: CapabilityBindingInput): string | null {
  const scope = (input.scope ?? '').trim() || 'tenant';
  if (scope !== 'tenant' && scope !== 'global') return `scope 必须是 tenant 或 global(得到 ${scope})`;
  if (scope === 'tenant' && !((input.tenant_id ?? 0) > 0)) return 'tenant scope 的绑定需要正的 tenant_id';
  if ((input.capability ?? '').trim() === '') return 'capability 不能为空';
  return null;
}

// buildCapabilityBindingBody: 精确 key-set —— scope/capability/enabled 必带(**无 source**, 防伪装 vendor-sync);
// capability_value/capability_params/tenant_id 仅显式给时附带。enabled 永远显式带(必填, 防后端按零值静默翻 disabled)。
export function buildCapabilityBindingBody(input: CapabilityBindingInput): Record<string, unknown> {
  const body: Record<string, unknown> = {
    scope: (input.scope ?? '').trim() || 'tenant',
    capability: input.capability,
    enabled: input.enabled,
  };
  if (input.capability_value !== undefined) body.capability_value = input.capability_value;
  if (input.capability_params !== undefined) body.capability_params = input.capability_params;
  if (input.tenant_id !== undefined) body.tenant_id = input.tenant_id;
  return body;
}
...
export const MODEL_ADMIN_ERROR_MESSAGES: Record<string, string> = {
  ...
  invalid_capability_binding: '能力绑定请求非法(enabled 字段必填)',
