...
var gaps = []gap{
	{"FE-GAP-001", "P0", "FE-PG-006", "音频协议", "部分交付时的计费、补偿和人工恢复合同", "后端已挂载部分交付待对账标记，不得只凭成功响应标记已结算；专用对账与人工恢复仍未收口该标记"},
	{"FE-GAP-002", "P1", "FE-PG-002", "运营聚合", "管理员与租户作用域的 Token、缓存、成本趋势、账号批量指标、池健康和链路图投影", "后端已挂载租户小时趋势、缓存构成、按池健康快照与按请求链路投影；前端待接线。禁止浏览器跨权限拼装。全平台小时拆分、缓存维度排行、标准成本/毛利、活跃租户周期对比、Top 用户趋势、账号批量指标另切"},
	{"FE-GAP-003", "P1", "FE-PG-003", "账号批量运维", "按选中账号执行批量动作与即时凭据刷新", "后端已挂载 `POST /admin/v1/provider-accounts/bulk`；前端待接线。按标签批量不能替代本入口"},
	{"FE-GAP-004", "P1", "FE-PG-008", "内容安全", "审核规则覆盖适用的全部兄弟协议入口", "后端已覆盖聊天族与兄弟协议的用户可控文本闸；二进制媒体模型级审核仍是强制路线，前端待接线"},
	{"FE-GAP-005", "P1", "FE-PG-008", "Worker 运维", "模型同步和订阅 Worker 的节点、领导者、最后 tick 与集群口径", "后端已挂载跨副本心跳与执行者投影；前端待接线。心跳面不可用返回 503"},
	{"FE-GAP-006", "P1", "FE-PG-003", "代理运维", "持久质量检测、多目标质量、批量管理和备用代理", "后端已挂载持久质量快照（延迟/错误类/等级/新鲜度）；多目标、批量添加与备用切换另切。不得以单次连通或静默直连冒充完整代理恢复"},
	{"FE-GAP-007", "P1", "FE-PG-002/FE-PG-007", "租户运营", "租户管理员的本租户经营总览和运行日志投影", "后端已挂载租户作用域经营总览；前端待接线。租户管理员尚无本租户运行日志入口（现入口仅部署者）"},
...
func classify(op operation, base tagSpec) (page, scene string, integration bool) {
	...
	case strings.Contains(path, "/admin/v1/usage/cache-composition"):
