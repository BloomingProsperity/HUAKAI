...
func cleanupSpecializedLiveMoneyRows(
	...
	for _, statement := range []string{
		...
		`DELETE FROM audit_refund_pending WHERE tenant_id=$1`,
		`DELETE FROM settlement_intents WHERE tenant_id=$1`,
		`ALTER TABLE billing_refund_operations DISABLE TRIGGER billing_refund_operations_append_only_delete`,
		`DELETE FROM billing_refund_operations WHERE tenant_id=$1`,
		`ALTER TABLE billing_refund_operations ENABLE TRIGGER billing_refund_operations_append_only_delete`,
		`ALTER TABLE billing_ledger_adjustments DISABLE TRIGGER billing_ledger_adjustments_append_only_delete`,
		`DELETE FROM billing_ledger_adjustments WHERE tenant_id=$1`,
		`ALTER TABLE billing_ledger_adjustments ENABLE TRIGGER billing_ledger_adjustments_append_only_delete`,
