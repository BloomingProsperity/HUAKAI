...
// 客户端 API 工作流测试：启动真实网关和 dev-mock 上游，精确驱动
// 用户与管理客户端依赖的端点、请求形状和响应字段。每个业务域一个子测试。
// 如果后端改动稳定字段、混用认证方式或漏接路由，对应子测试必须变红。
//
// 复用 smoke_test.go 里的 smoke 测试脚手架
//(buildGateway/startGateway/seedSmokeGraph/...)。运行：
// go test -tags smoke -run TestClientAPIWorkflow ./cmd/gateway
...
func TestClientAPIWorkflow(t *testing.T) {
	...
	if dsn == "" {
		t.Fatal("HUAKAI_DATABASE_URL 未配置，客户端 API 工作流测试拒绝假绿")
	}
	...
	for k, v := range map[string]string{
		...
		if _, err := pgPool.Exec(setupCtx,
			`INSERT INTO platform_settings (scope, setting_key, setting_value, updated_by, updated_at)
			 VALUES ('global',$1,$2,'client-api-workflow-test', now())
			 ON CONFLICT (scope, setting_key) DO UPDATE SET setting_value=$2, updated_by='client-api-workflow-test', updated_at=now()`, k, v); err != nil {
			t.Fatalf("seed platform_setting %s: %v", k, err)
		}
	}

	configureClientAPIWorkflowHermes(t)
	...
	// ---- 登录工作流（register → login → me）----
	t.Run("login_api_auth_ring", func(t *testing.T) {
		// register(public)：稳定请求为 {tenant_id,email,display_name,password}。
		...
		if !ok {
			t.Fatalf("login: response missing `session` object; body=%s", body)
		}
	...
	// ---- API Key 工作流（create → list）----
	t.Run("api_keys_api", func(t *testing.T) {
		// create：客户端只在本次响应读取一次性的 `plaintext` 字段。
		...
		if !ok {
			t.Fatalf("list api-keys: missing `api_keys` array; body=%s", body)
		}
		if len(arr) == 0 {
		...
		// 单 Key 用量汇总使用 SESSION 认证，读取 api_key_id/total_cost/request_count。
		...
		for _, f := range []string{"api_key_id", "total_cost", "request_count"} {
			if _, ok := sum[f]; !ok {
				t.Fatalf("usage-summary missing %q; body=%s", f, body)
			}
		}
	...
	// ---- 用量工作流（quota[session] + usage[apikey] + time-series[apikey]）----
	t.Run("usage_api", func(t *testing.T) {
		// /v1/me/quota 使用 SESSION 认证，稳定信封为 { items: [...] }。
		...
		if _, ok := q["items"]; !ok {
			t.Fatalf("/v1/me/quota: missing `items`; body=%s", body)
		}

		// /v1/me/usage 使用 API Key 认证，稳定信封为 { items, next_cursor }。
	...
	// ---- 推理工作流（models[apikey] + chat stream[apikey]）----
	t.Run("inference_api", func(t *testing.T) {
		// 用 seed key 调 GET /v1/models，稳定信封为 { object:"list", data:[{id}] }。
		...
		// POST /v1/chat/completions 流式发送路径（SSE + usage）。
	...
	// ============ 批次 1：最终用户 API（session 认证）============
	// 刚注册的用户数据为空，但每个端点都必须返回 200 和稳定信封。为
	// redeem / subscriptions / notifications / account
	// 断言接线(路由已挂载、认证被接受、形状正确)。

	// 模块:redeem(vouchers)—— history GET + redeem 错误路径(路由已接线)。
	t.Run("redeem_api", func(t *testing.T) {
	...
	t.Run("subscriptions_api", func(t *testing.T) {
	...
	t.Run("notifications_api", func(t *testing.T) {
	...
	// 每个依赖都必须由生产网关完整注入并返回稳定的 200 响应。
	t.Run("account_api", func(t *testing.T) {
	...
	t.Run("billing_api", func(t *testing.T) {
	...
	t.Run("security_api", func(t *testing.T) {
	...
	t.Run("pricing_api", func(t *testing.T) {
	...
	t.Run("receipt_and_log_api", func(t *testing.T) {
	...
	t.Run("admin_system_api", func(t *testing.T) {
	...
	t.Run("sessions_api", func(t *testing.T) {
		...
		if _, ok := obj["families"]; !ok {
			t.Fatalf("sessions list missing `families`; body=%s", body)
		}
	})

	// 模块:hermes(管理助手)。本测试显式装配测试运行器身份，使生产组合根必须挂载
	// /v1/hermes；身份和租户完全从管理员凭据推导，不允许用缺少依赖掩盖 404。
	t.Run("hermes_api", func(t *testing.T) {
	...
	t.Run("specialized_inference_api", func(t *testing.T) {
...
func configureClientAPIWorkflowHermes(t *testing.T) {
	t.Helper()
	...
	t.Setenv(hermes.RunnerJWTKIDEnv, "client-api-workflow")
	t.Setenv(hermeschat.InternalTokenSecretEnv, "client-api-workflow-internal-token-secret")
}
