-- 统一恢复原语(RecoverProviderAccountState)写管理员审计时用新 action
-- recover_provider_account_state;显式扩展 action 白名单(逐字复现 0181 + 新增该项),
-- 否则真实库的 admin_audit_events_action_check 会拒绝审计行、令恢复端点整体失败。
BEGIN;
