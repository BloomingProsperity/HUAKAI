...
-- 发现箱是运营事实。只要存在任何记录就拒绝降级，要求先显式迁移，而不是在
-- schema 回退时静默删除待处理、已忽略或已上架的决策证据。
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM model_discovery_inbox LIMIT 1) THEN
        RAISE EXCEPTION '不能回滚 0201：模型发现箱仍有记录，请先显式迁移这些运营事实';
    END IF;
END $$;

DROP TABLE model_discovery_inbox;
