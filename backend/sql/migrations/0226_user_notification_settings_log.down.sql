...
BEGIN
    IF EXISTS (
        ...
        WHERE action = 'update_user_notification_settings'
           OR target_type = 'user_notification_settings'
    ) THEN
        RAISE EXCEPTION
            '不能回滚 0226：仍存在用户通知设置安全日志，请先显式迁移这些运营事实';
    END IF;
END
$$;
...
ALTER TABLE admin_audit_events
    ...
    ADD CONSTRAINT admin_audit_events_action_check CHECK (action = ANY (ARRAY[
        ...
        'test_provider_account', 'update_model_capability_binding',
