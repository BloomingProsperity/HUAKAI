-- 0161 down: 把三处 CHECK 还原到 0161 之前的状态(即 0160 的完整列表)——
--   (1) hermes_tool_calls.tool_name 还原到 0160 的 21 工具名(移除 moderation_keyword_enable/disable);
--   (2) admin_audit_events.action 还原到 0160 的完整 action 列表(移除两个 hermes.tool.moderation_keyword_*);
--   (3) admin_audit_events.target_type 还原到 0160 的列表(移除 moderation_keyword,保留 alert_rule)。
-- 逐字复现各自 0161 之前的 IN-list。
