...
BEGIN
    IF EXISTS (
        ...
        WHERE action IN ('create_tenant', 'enable_tenant', 'disable_tenant', 'delete_tenant')
    ) THEN
        RAISE EXCEPTION '不能回滚 0233：仍存在租户生命周期操作日志，请先显式迁移这些事实';
    END IF;
END
$$;

DROP INDEX IF EXISTS idx_tenants_lifecycle;

ALTER TABLE tenants
    DROP CONSTRAINT IF EXISTS tenants_lifecycle_check,
    DROP CONSTRAINT IF EXISTS tenants_status_reason_length_check,
    DROP CONSTRAINT IF EXISTS tenants_version_check,
