...
BEGIN
    IF EXISTS (
        ...
        WHERE action IN ('move_provider_account_channel', 'bulk_provider_accounts')
           OR target_type = 'provider_account_batch'
    ) THEN
        RAISE EXCEPTION
            '不能回滚 0240:仍存在账号批量运维日志,请先显式迁移这些运营事实';
    END IF;
END $$;
    ...
    ADD CONSTRAINT admin_audit_events_target_type_check
        CHECK (target_type IN
            ('api_key', 'admin_token', 'tenant', 'user',
             ...
             'channel_test_template', 'media_task_orphan'));
