...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM subscription_fulfillment_effects LIMIT 1) THEN
        RAISE EXCEPTION 'refuse rollback 0075: subscription_fulfillment_effects has rows'
            USING ERRCODE = 'check_violation';
    END IF;
    IF EXISTS (SELECT 1 FROM payment_orders WHERE order_kind = 'subscription' LIMIT 1) THEN
        RAISE EXCEPTION 'refuse rollback 0075: payment_orders has subscription orders'
            USING ERRCODE = 'check_violation';
    END IF;
    IF EXISTS (SELECT 1 FROM voucher WHERE grant_kind = 'subscription' LIMIT 1) THEN
        RAISE EXCEPTION 'refuse rollback 0075: voucher has subscription grants'
            USING ERRCODE = 'check_violation';
    END IF;
END $$;
