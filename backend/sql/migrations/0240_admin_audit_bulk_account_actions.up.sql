-- 0240:账号批量运维的管理日志动作与目标类型(纯加性白名单扩展,不动任何数据)。
-- move_provider_account_channel:逐项改渠道;bulk_provider_accounts:批次级日志(子动作、选择形状与计数在 payload)。
-- provider_account_batch:批次级日志的目标类型(无单一 target_id)。
...
ALTER TABLE admin_audit_events
    ...
    ADD CONSTRAINT admin_audit_events_action_check CHECK (action = ANY (ARRAY[
        ...
        'move_provider_account_channel', 'bulk_provider_accounts'
    ]::text[]));
    ...
    ADD CONSTRAINT admin_audit_events_target_type_check
        CHECK (target_type IN
            ('api_key', 'admin_token', 'tenant', 'user',
             ...
             'account_bundle', 'model_capability_binding',
             'user_notification_settings', 'route_rule',
             'proxy', 'model_pool_binding', 'model_routing_override',
             'channel_test_template', 'media_task_orphan', 'provider_account_batch'));
