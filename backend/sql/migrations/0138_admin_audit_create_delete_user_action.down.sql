BEGIN;
-- 回滚到 0136 的 action 集合(移除 create_user / delete_user)。down 前提:无
-- action IN ('create_user','delete_user') 的存量行(否则 ADD CHECK 失败)——
-- 回滚窗口内若已写入须先清理,与本仓既有 audit-action 迁移的 down 约定一致。
