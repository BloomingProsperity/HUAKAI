BEGIN;
-- 加性扩展 admin_audit_events 白名单(配额策略 admin CRUD,BILL-122):
--   action 新增 create_quota_policy / update_quota_policy / delete_quota_policy
--   target_type 新增 quota_policy
-- 镜像 0138(action)/0077(target_type)的 DROP+ADD CHECK 形状;纯加性,不动既有值。
-- 不加此白名单,adminquotahttp 的 create/update/delete 在同 tx 写审计行时会触发 CHECK 违反。
