...
func TestIssueSignupCredits_EnqueuesOnlyFailedRewards(t *testing.T) {
	...
	s := &Service{
		...
		SignupRewardRecoveryFn: func(_ context.Context, tenantID, userID int64, kind string) error {
...
func TestIssueSignupCredits_RecoverySurvivesRequestCancellation(t *testing.T) {
	...
	s := &Service{
		SignupBonusFn: func(context.Context, int64, int64) error { return errors.New("bonus down") },
		SignupRewardRecoveryFn: func(recoveryCtx context.Context, tenantID, userID int64, kind string) error {
			called = true
			if recoveryCtx.Err() != nil {
				t.Fatalf("恢复入队继承了已取消请求: %v", recoveryCtx.Err())
			}
			if tenantID != 7 || userID != 42 || kind != "signup_bonus" {
				t.Fatalf("恢复事件=%d/%d/%s，期望 7/42/signup_bonus", tenantID, userID, kind)
			}
			return nil
		},
	}
