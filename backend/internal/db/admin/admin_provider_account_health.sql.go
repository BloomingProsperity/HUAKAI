...
type SummarizeProviderAccountHealthByPoolRow struct {
	...
	AuthCooldownAccounts int64              `db:"auth_cooldown_accounts" json:"auth_cooldown_accounts"`
	EarliestRecoveryAt   pgtype.Timestamptz `db:"earliest_recovery_at" json:"earliest_recovery_at"`
}
...
func (q *Queries) SummarizeProviderAccountHealthByPool(ctx context.Context, tenantID int64) ([]SummarizeProviderAccountHealthByPoolRow, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&i.AuthCooldownAccounts,
			&i.EarliestRecoveryAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
