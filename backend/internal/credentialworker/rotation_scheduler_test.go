...
// flags the due candidates. Mutation guard: drop the ScanRotationDue call in
// RunOnce and the store is never touched → red.
func TestRunOnce_RunsRotationScanWhenEnabled(t *testing.T) {
	store := &fakeRotationStore{due: []RotationCandidate{{CredentialID: 1, TenantID: 2, ProviderAccountID: 3}}}
	...
	if store.gotLimit != 10 || len(store.flagged) != 1 {
		t.Fatalf("enabled rotation scan must query(limit=10)+flag(1), got limit=%d flagged=%d", store.gotLimit, len(store.flagged))
	}
...
func TestRunOnce_SkipsRotationScanByDefault(t *testing.T) {
	store := &fakeRotationStore{due: []RotationCandidate{{CredentialID: 1}}}
	...
	if store.gotLimit != 0 || len(store.flagged) != 0 {
		t.Fatalf("maxAge=0 must skip the rotation scan, got limit=%d flagged=%d", store.gotLimit, len(store.flagged))
	}
}
