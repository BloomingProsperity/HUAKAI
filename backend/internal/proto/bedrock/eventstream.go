// proto/bedrock/eventstream.go — Bedrock-on-Anthropic 上游事件适配器（A4）。
//
// 上下文：
...
//   - 独立 struct（不直接复用 anthropic.Adapter）：未来 Bedrock-on-Llama /
//     Bedrock-on-Cohere 不能 delegate Anthropic；matrix 上 Bedrock 的 loss
//     attribution 与 native Anthropic 不同（虽当前等价，但语义不同）
//   - 但内部实现 delegate `*anthropic.Adapter`：Bedrock-on-Anthropic 的 inner
//     payload IS Anthropic 事件 JSON，逐字节复用 anthropic.Adapter 的解析逻辑
...
// EventStreamAdapter 把 A3 scanner 产出的 SSEEvent payload（内层
// Anthropic 事件 JSON）翻译为 proto.HCSF CanonicalEvent。
//
// 实现策略：本 adapter 是**完全无状态**的（仅 immutable policy 字段）。
// 每次调用 ProviderEventToCanonicalEvents / FinalizeUpstreamStream 时
// 构造 local anthropic.Adapter delegate — 因为 Bedrock-on-Anthropic 的
...
type EventStreamAdapter struct {
	// CarryForwardSignatureDelta 透传给 per-call 构造的 anthropic.Adapter；
	// 当前 Bedrock 路径默认 false（与 capability matrix 一致）。
	CarryForwardSignatureDelta bool
}

// NewEventStreamAdapter 构造 Bedrock 上游事件 adapter。
// 当前等价于 &EventStreamAdapter{}（零值可用），保留构造函数
// 以防未来加内部 init 步骤。
func NewEventStreamAdapter() *EventStreamAdapter {
	return &EventStreamAdapter{}
}

// innerDelegate 返回本次调用专用的 anthropic.Adapter（per-call 构造，
// 无并发 race 风险）。
func (s *EventStreamAdapter) innerDelegate() *anthropic.Adapter {
	return &anthropic.Adapter{CarryForwardSignatureDelta: s.CarryForwardSignatureDelta}
}

// CanonicalToProviderRequest 占位：A8 才实现 OpenAI→Bedrock-Anthropic
// 请求 body 翻译（R1 决策：本路径优先级 P0，但 A8 才动）。
func (s *EventStreamAdapter) CanonicalToProviderRequest(ctx context.Context, canonical *proto.HCSF) ([]byte, []proto.ProtocolLossEntry, error) {
...
//
// state 必须是 *anthropic.UpstreamState（与 anthropic.Adapter 共享，因为 Bedrock-on-
// Anthropic 的 stream 状态机与 native Anthropic 一致）。
//
// **并发安全**：本 adapter 无 mutable 字段，多 goroutine 共享调用安全；
// 但 state（*anthropic.UpstreamState）必须每流独立——这是接口契约，不是 adapter 责任。
func (s *EventStreamAdapter) ProviderEventToCanonicalEvents(ctx context.Context, providerEvt any, state any) ([]any, []proto.ProtocolLossEntry, error) {
	if _, ok := state.(*anthropic.UpstreamState); !ok {
		return nil, nil, fmt.Errorf("proto: EventStreamAdapter expected *anthropic.UpstreamState")
	}
	// per-call 构造 — 无 race。anthropic.Adapter 自身 stateless 除 immutable bool。
