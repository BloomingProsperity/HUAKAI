...
func TestAT_AUDIT_001_056_ResolvedDisputeRefundsLedgerAndBalance(t *testing.T) {
	...
	assertDisputeRefundState(t, ctx, pool, f, DisputeStatusResolved, 1, decimal.RequireFromString("10.01234500"))
...
func TestAT_AUDIT_001_058_RejectedDisputeDoesNotTouchMoney(t *testing.T) {
	...
	assertDisputeRefundState(t, ctx, pool, f, DisputeStatusRejected, 0, decimal.RequireFromString("10.00000000"))
}
...
RETURNING id`, f.tenantID, f.userID, "key-"+suffix, "$2a$10$dispute-refund-placeholder", "hk_test_"+suffix[:8]).Scan(&f.apiKeyID); err != nil {
	...
	if _, err := pool.Exec(ctx, `INSERT INTO user_balances (tenant_id, user_id, balance, held) VALUES ($1, $2, 10, 0)`, f.tenantID, f.userID); err != nil {
