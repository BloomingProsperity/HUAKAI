...
func NewEmbeddingsHandler(d Deps) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()
		if !configured(d) {
			writeJSONError(w, http.StatusServiceUnavailable, "gateway_not_configured", "embeddings handler dependency unset")
			return
		}
		...
		body, req, texts, ok := validateRequest(w, r)
		if !ok {
			return
		}
...
func (ex *execution) screenInbound(w http.ResponseWriter) bool {
	return moderation.ApplyHTTP(w, ex.ctx, ex.d.ModerationScreener, moderation.ScreenRequest{
		...
		ClientProtocol: moderation.ProtocolOpenAIEmbeddings,
		Body:           ex.body,
	})
}
