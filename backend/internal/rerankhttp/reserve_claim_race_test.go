...
// TestRerankHandler_ReserveClaimRaceReturns409RetryAfter 守 reserve 阶段的
// ErrClaimRace 分支:命中可重试竞争必须回 409 + Retry-After:1 + code=claim_race,
// 而非通用 500 reserve_error。复用 rerankClaimGate 已有的 err 注入位。
// 变异契约:删掉 billing.go reserve() 里 `if errors.Is(err, billing.ErrClaimRace)`
// 分支后,请求会落到下方通用 500 reserve_error,本测试随即变红。
func TestRerankHandler_ReserveClaimRaceReturns409RetryAfter(t *testing.T) {
