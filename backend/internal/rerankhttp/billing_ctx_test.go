...
// 判别测试:请求 ctx 已取消时 abort 必须仍以未取消的 billingCtx 调 Settler.Abort。
// Mutation guard: abort 改回 ex.ctx → abortCtxErr==context.Canceled → 红。
func TestRerankAbort_SurvivesClientDisconnect(t *testing.T) {
...
func TestRerankBillingCtx_DetachedWithDeadline(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
