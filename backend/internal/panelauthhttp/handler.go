...
func MountAuthMeRoutes(r chi.Router, d Deps) {
	r.Get("/me", newAuthMeHandler(d))
}

func newAuthMeHandler(d Deps) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if d.Resolver == nil {
			writeJSONError(w, http.StatusServiceUnavailable, "gateway_not_configured", "panel auth dependency unset")
			return
		}
		...
		if err != nil {
			...
			writeJSONError(w, http.StatusServiceUnavailable, "panel_backend_error", "panel resolution unavailable")
			return
		}
		writeJSON(w, http.StatusOK, meResponse{Panel: string(panel), UserID: ident.UserID, TenantID: ident.TenantID})
	}
}
