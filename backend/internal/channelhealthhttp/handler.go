...
func newChannelHealthSummaryHandler(d ChannelHealthAdminDeps) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		writeJSON(w, http.StatusOK, channelHealthSummaryResponse(summary))
	}
}
...
func channelHealthMutationHandler(
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		providerAccountID, ok := adminhttpcore.ParseRequiredPositiveInt64(
			w,
			chi.URLParam(r, "id"),
		...
		writeJSON(w, http.StatusOK, channelHealthResponse(rec))
	}
}
