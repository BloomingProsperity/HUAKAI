...
// retryTx2 在完整资金事务遇到 Serializable 冲突时有限重试。
// fn 必须自包含 BeginTx→Commit/Rollback,所有 usage_record、billing_event、hold
// capture/release、余额和 pool slot 写入都必须在该事务内完成。PostgreSQL 对
// 40001/40P01 会撤销整事务,所以重跑不会留下部分写入。结算和中止依靠 claim
// 终态守卫收敛，退款依靠 claim 行锁、不可变退款事实与累计上限收敛。
func retryTx2(
	ctx context.Context,
	name string,
