...
//
// 当前写入沿用 last_probe_at 旧存储列;管理 API 以 last_request_observed_at 暴露真实语义。
// 该写入纯可观测,不碰钱 / auth / health_state,不引入 schema 迁移。
...
// ProbeStore 是被动请求观测写入所需的最小存储面。生产实现是 *admin.Queries。
