...
func (s *Service) ApplySignal(ctx context.Context, sig Signal) (Record, error) {
	...
	//(车道自有真相表 + 进程镜像),完全不改 rec.State/Score、不写健康窗口(防 auth blip 污染健康分,
	// 保留既有「令牌问题不写健康降级」意图)。刻意在 withMutation 之前短路:该路径不碰健康存储,
	// 进 Serializable 事务只会白开空事务——401 风暴(恰是车道目标场景)下每失败一次多两趟 DB
	// 往返;车道自身的一条原子 upsert 在窗口内不产生写入。lane 未接线(开关关)时同样短路,
	// 与基底「auth 类直接跳过」逐字节等价。
...
func (s *Service) applySignal(ctx context.Context, sig Signal) (Record, error) {
	...
	// 成功信号顺带清 auth 车道的软退避(一次成功即解除临时冷却、strike 归零);硬禁不受成功影响,
	// 只由运营 resume / 显式凭据轮换解除。
