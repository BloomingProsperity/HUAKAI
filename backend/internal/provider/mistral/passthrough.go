// 包 mistral — Mistral AI 平台的出站请求适配器。
//
// Mistral 走 OpenAI Chat Completions 兼容协议；endpoint 为
// api.mistral.ai/v1/chat/completions。鉴权用 Mistral 官方
...
// 默认 Mistral 官方 Chat Completions endpoint。
const defaultChatCompletionsEndpoint = "https://api.mistral.ai/v1/chat/completions"

// PassthroughAdapter 把客户原始 OpenAI 形态请求直通转发到 Mistral。
type PassthroughAdapter struct {
	// Endpoint 覆盖默认 endpoint（如自托管 OpenAI-compatible 代理前置）。
	Endpoint string
}

func (a *PassthroughAdapter) Platform() string { return "mistral" }
...
func (a *PassthroughAdapter) BuildRequest(ctx context.Context, in provider.BuildInput) (*http.Request, error) {
	if !a.acceptsCredential(in.Credential.Type) {
		return nil, fmt.Errorf("mistral passthrough: 不支持的凭据形态 %q", in.Credential.Type)
	}
	if in.Credential.Value == "" {
		return nil, errors.New("mistral passthrough: 凭据 Value 为空")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("mistral passthrough: endpoint rejected: %w", err)
	}

	req, err := http.NewRequestWithContext(ctx, http.MethodPost, endpoint, bytes.NewReader(in.InboundBody))
	if err != nil {
		return nil, fmt.Errorf("mistral passthrough: 构造请求失败: %w", err)
	}
