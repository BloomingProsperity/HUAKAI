// Groq Cloud passthrough adapter 单元测试。
package groqcloud
...
func TestPassthroughAdapter_Platform(t *testing.T) {
	if got := (&PassthroughAdapter{}).Platform(); got != "groqcloud" {
...
func TestPassthroughAdapter_BuildRequest_APIKey(t *testing.T) {
	...
	in := provider.BuildInput{
		InboundBody: []byte(`{"model":"llama3-8b-8192","messages":[]}`),
		Credential: provider.Credential{
			Type:  provider.CredentialTypeAPIKey,
			Value: "gsk-test-fake-key",
		},
	}
	...
	if got := req.Header.Get("Authorization"); got != "Bearer gsk-test-fake-key" {
...
func TestPassthroughAdapter_BuildRequest_CustomEndpoint(t *testing.T) {
	a := &PassthroughAdapter{Endpoint: "https://my-groqcloud-proxy.example/chat"}
	in := provider.BuildInput{
		InboundBody: []byte(`{}`),
	...
	if req.URL.String() != "https://my-groqcloud-proxy.example/chat" {
		t.Errorf("URL=%q", req.URL.String())
	}
}
