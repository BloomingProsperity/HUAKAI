// Fireworks AI passthrough adapter 单元测试。
package fireworks
...
func TestPassthroughAdapter_Platform(t *testing.T) {
	if got := (&PassthroughAdapter{}).Platform(); got != "fireworks" {
...
func TestPassthroughAdapter_BuildRequest_APIKey(t *testing.T) {
	...
	in := provider.BuildInput{
		InboundBody: []byte(`{"model":"accounts/fireworks/models/llama-v3-8b-instruct","messages":[]}`),
		Credential: provider.Credential{
			Type:  provider.CredentialTypeAPIKey,
			Value: "fw-test-fake-key",
		},
	}
	...
	if req.URL.String() != "https://api.fireworks.ai/inference/v1/chat/completions" {
		t.Errorf("URL=%q", req.URL.String())
	}
	if got := req.Header.Get("Authorization"); got != "Bearer fw-test-fake-key" {
...
func TestPassthroughAdapter_BuildRequest_CustomEndpoint(t *testing.T) {
	a := &PassthroughAdapter{Endpoint: "https://my-fireworks-proxy.example/chat"}
	in := provider.BuildInput{
		InboundBody: []byte(`{}`),
	...
	if req.URL.String() != "https://my-fireworks-proxy.example/chat" {
		t.Errorf("URL=%q", req.URL.String())
	}
}
