// Perplexity AI passthrough adapter 单元测试。
package perplexity
...
func TestPassthroughAdapter_Platform(t *testing.T) {
	if got := (&PassthroughAdapter{}).Platform(); got != "perplexity" {
...
func TestPassthroughAdapter_BuildRequest_APIKey(t *testing.T) {
	...
	in := provider.BuildInput{
		InboundBody: []byte(`{"model":"llama-3.1-sonar-small-128k-online","messages":[]}`),
		Credential: provider.Credential{
			Type:  provider.CredentialTypeAPIKey,
			Value: "pplx-test-fake-key",
		},
	}
	...
	if got := req.Header.Get("Authorization"); got != "Bearer pplx-test-fake-key" {
...
func TestPassthroughAdapter_BuildRequest_CustomEndpoint(t *testing.T) {
	a := &PassthroughAdapter{Endpoint: "https://my-perplexity-proxy.example/chat"}
	in := provider.BuildInput{
		InboundBody: []byte(`{}`),
	...
	if req.URL.String() != "https://my-perplexity-proxy.example/chat" {
		t.Errorf("URL=%q", req.URL.String())
	}
}
