...
func (r *PostgresProxyResolver) Resolve(ctx context.Context, accountID int64) (*url.URL, error) {
	...
	if !row.hasProxyBound && row.proxyGroupID != "" {
		...
		grow := proxyRowFromFields(row.tenantID, *chosen)
	...
	chosen := proxyRowFromFields(row.tenantID, fields)
...
func proxyRowFromFields(tenantID int64, fields proxyFields) proxyRow {
	return proxyRow{
		tenantID: tenantID,
