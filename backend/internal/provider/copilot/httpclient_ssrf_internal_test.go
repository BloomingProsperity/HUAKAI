...
// Copilot service-token 刷新出站携带高价值 GitHub access token。未注入 HTTPClient 时，生产
// 构造点必须回退到 SSRF 防护 client，而非裸
// http.DefaultClient(后者读 HTTP_PROXY 经 env 代理外发 token、无拨号层 IP 校验、不禁 3xx)。
// §14 变异:把 httpClient() 改回 `return http.DefaultClient` → 其 Transport 为 nil(断言 ok=false)→ 本测试红。
