...
func TestPassthroughAdapter_Platform(t *testing.T) {
	if got := (&PassthroughAdapter{}).Platform(); got != "grok" {
...
func TestPassthroughAdapter_BuildRequest_APIKey(t *testing.T) {
	...
	in := provider.BuildInput{
		InboundBody: []byte(`{"model":"grok-2","messages":[]}`),
		Credential: provider.Credential{
			Type:  provider.CredentialTypeAPIKey,
			Value: "xai-test-fake-key",
		},
	}
	...
	if got := req.Header.Get("Authorization"); got != "Bearer xai-test-fake-key" {
...
func TestPassthroughAdapter_BuildRequest_CustomEndpoint(t *testing.T) {
	a := &PassthroughAdapter{Endpoint: "https://my-grok-proxy.example/chat"}
	in := provider.BuildInput{
		InboundBody: []byte(`{}`),
	...
	if req.URL.String() != "https://my-grok-proxy.example/chat" {
		t.Errorf("URL=%q", req.URL.String())
	}
}
