// Together AI passthrough adapter 单元测试。
package together
...
func TestPassthroughAdapter_Platform(t *testing.T) {
	if got := (&PassthroughAdapter{}).Platform(); got != "together" {
...
func TestPassthroughAdapter_BuildRequest_APIKey(t *testing.T) {
	...
	in := provider.BuildInput{
		InboundBody: []byte(`{"model":"meta-llama/Llama-3-8b-chat-hf","messages":[]}`),
		Credential: provider.Credential{
			Type:  provider.CredentialTypeAPIKey,
			Value: "ta-test-fake-key",
		},
	}
	...
	if req.URL.String() != "https://api.together.xyz/v1/chat/completions" {
		t.Errorf("URL=%q", req.URL.String())
	}
	if got := req.Header.Get("Authorization"); got != "Bearer ta-test-fake-key" {
...
func TestPassthroughAdapter_BuildRequest_CustomEndpoint(t *testing.T) {
	a := &PassthroughAdapter{Endpoint: "https://my-together-proxy.example/chat"}
	in := provider.BuildInput{
		InboundBody: []byte(`{}`),
	...
	if req.URL.String() != "https://my-together-proxy.example/chat" {
		t.Errorf("URL=%q", req.URL.String())
	}
}
