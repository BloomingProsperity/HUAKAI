...
const (
	CursorVendor        = "cursor"
	CursorAuthModeOAuth = "oauth"

	CursorPKCEMethodS256 = "S256"

	DefaultCursorOAuthRedirectURI = "http://127.0.0.1:1455/auth/callback"
)

var ErrCursorOAuthConfigRequired = errors.New("cursor oauth: operator-verified OAuth config required")

func DefaultOAuthConfig() credentialacq.OAuthClientConfig {
	return credentialacq.OAuthClientConfig{
		RedirectURI: DefaultCursorOAuthRedirectURI,
...
func ValidateOAuthConfig(cfg credentialacq.OAuthClientConfig) error {
	...
	if len(missing) > 0 {
		return fmt.Errorf("%w: missing %s", ErrCursorOAuthConfigRequired, strings.Join(missing, ","))
	}
	return nil
}
...
func BuildOAuthAuthorizeURL(cfg credentialacq.OAuthClientConfig, state, codeChallenge string) (string, error) {
	...
	if raw == "" {
		return "", fmt.Errorf("%w: authorize URL could not be built", ErrCursorOAuthConfigRequired)
	}
	return raw, nil
}
