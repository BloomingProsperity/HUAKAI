...
const (
	...
	geminiSessionMode     = "gemini_session"
	geminiOAuthModeAlias  = "gemini_oauth"
)

var (
	ErrGeminiStoreMissing   = errors.New("gemini refresh: credential store missing")
	ErrGeminiAuthExpired    = errors.New("gemini refresh: authorization expired")
	ErrGeminiRateLimited    = errors.New("gemini refresh: rate limit exceeded")
	ErrGeminiRiskControl    = errors.New("gemini refresh: risk control triggered")
	ErrGeminiTransient      = errors.New("gemini refresh: transient upstream failure")
	ErrGeminiRecordMismatch = errors.New("gemini refresh: credential record is not gemini oauth")
)
...
func (r *Refresher) refresh(ctx context.Context, _ int64, accountID int64) error {
	...
	if !isGeminiOAuthRecord(probe) {
		return fmt.Errorf("%w: vendor=%s auth_mode=%s account_id=%d", ErrGeminiRecordMismatch, probe.Vendor, probe.AuthMode, accountID)
	}
...
func (r *Refresher) refreshLockedRecord(ctx context.Context, txStore RefreshTxStore, accountID int64, rec credentialstore.CredentialRecord) (error, error) {
	if !isGeminiOAuthRecord(rec) {
		err := fmt.Errorf("%w: vendor=%s auth_mode=%s account_id=%d", ErrGeminiRecordMismatch, rec.Vendor, rec.AuthMode, accountID)
		if saveErr := txStore.SaveRefreshFailure(ctx, rec, failureVendorMismatch, r.now().Add(defaultRetryAfter)); saveErr != nil {
			return nil, saveErr
		}
		return err, nil
	}
	payload, expiresAt, err := r.Adapter.RefreshForProvider(ctx, accountID, GeminiVendor, rec.PlaintextPayload)
	if err != nil {
...
func (a RefreshAdapter) RefreshForProvider(ctx context.Context, accountID int64, _ string, currentCredential []byte) ([]byte, time.Time, error) {
	...
	if refreshToken == "" {
		return nil, time.Time{}, fmt.Errorf("gemini refresh account %d: %w: refresh_token is empty", accountID, credentialstore.ErrInvalidPayload)
	}
	tokenURL := strings.TrimSpace(a.TokenURL)
	if tokenURL == "" {
		return nil, time.Time{}, fmt.Errorf("gemini refresh account %d: %w: token_url", accountID, ErrGeminiOAuthConfigRequired)
	}
	clientID := strings.TrimSpace(a.ClientID)
	if clientID == "" {
		return nil, time.Time{}, fmt.Errorf("gemini refresh account %d: %w: client_id", accountID, ErrGeminiOAuthConfigRequired)
	}
	scope := strings.TrimSpace(a.Scope)
	if scope == "" {
		return nil, time.Time{}, fmt.Errorf("gemini refresh account %d: %w: scope", accountID, ErrGeminiOAuthConfigRequired)
	}
	form := url.Values{}
...
func isGeminiOAuthRecord(rec credentialstore.CredentialRecord) bool {
	if credentialstore.Normalize(rec.Vendor) != GeminiVendor {
		return false
	}
	switch credentialstore.Normalize(rec.AuthMode) {
	case GeminiAuthModeOAuth, geminiOAuthModeAlias, geminiSessionMode,
		credentialstore.AuthModeCodeAssist, credentialstore.AuthModeGoogleOne:
		return true
	default:
		return false
	}
