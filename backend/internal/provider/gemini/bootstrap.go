...
const (
	GeminiVendor        = "gemini"
	GeminiAuthModeOAuth = "oauth"

	GeminiPKCEMethodS256 = "S256"

	DefaultGeminiOAuthRedirectURI = "http://127.0.0.1:1455/auth/callback"
)

var ErrGeminiOAuthConfigRequired = errors.New("gemini oauth: operator-verified OAuth config required")

func DefaultOAuthConfig() credentialacq.OAuthClientConfig {
	return credentialacq.OAuthClientConfig{
		RedirectURI: DefaultGeminiOAuthRedirectURI,
...
func ValidateOAuthConfig(cfg credentialacq.OAuthClientConfig) error {
	...
	if len(missing) > 0 {
		return fmt.Errorf("%w: missing %s", ErrGeminiOAuthConfigRequired, strings.Join(missing, ","))
	}
	return nil
}
...
func BuildOAuthAuthorizeURL(cfg credentialacq.OAuthClientConfig, state, codeChallenge string) (string, error) {
	...
	if raw == "" {
		return "", fmt.Errorf("%w: authorize URL could not be built", ErrGeminiOAuthConfigRequired)
	}
	return raw, nil
}
