...
func writeControlsError(w http.ResponseWriter, err error) {
	...
	case errors.Is(err, userkeycontrols.ErrInvalidIPBlacklist):
		writeError(w, http.StatusBadRequest, "invalid_ip_blacklist", "ip_blacklist entries must be CIDR or IP strings")
