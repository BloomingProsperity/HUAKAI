...
func (b *Bridge) PrepareRequest(ctx context.Context, req Request) (PreparedRequest, error) {
	...
	if requestID == "" {
		requestID = fmt.Sprintf("hermes-%d-%d-%d", req.TenantID, req.UserID, b.now().UTC().UnixNano())
	}
	token, err := SignInternalToken(b.internalTokenSecret, InternalTokenClaims{
		TenantID: req.TenantID, UserID: req.UserID, RequestID: requestID,
		ExpiresAt: b.now().UTC().Add(InternalTokenTTL),
	})
	if err != nil {
		return PreparedRequest{}, err
	}
