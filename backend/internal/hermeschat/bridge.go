...
func (b *Bridge) Stream(ctx context.Context, w http.ResponseWriter, resp *http.Response, prepared PreparedRequest) error {
	...
	if err := scanner.Err(); err != nil {
		if !state.terminal {
			state.terminal = true
			state.blocked = true
			b.recordStreamFailure(prepared, "runner_stream_read_failed")
			_ = writeRunnerError(w, flusher, "runner_stream_read_failed")
		}
		return err
	}
	if block.Len() > 0 {
	...
	if !state.terminal {
		state.terminal = true
		state.blocked = true
		b.recordStreamFailure(prepared, "runner_stream_incomplete")
		return writeRunnerError(w, flusher, "runner_stream_incomplete")
	}
	return nil
}
...
func writeRunnerError(w io.Writer, flusher http.Flusher, errorClass string) error {
	if strings.TrimSpace(errorClass) == "" {
		errorClass = "runner_failed"
	}
	return writeAndFlush(w, flusher, []byte(fmt.Sprintf("event: error\ndata: {\"code\":%q,\"message\":\"hermes agent failed\"}\n\n", errorClass)))
}
