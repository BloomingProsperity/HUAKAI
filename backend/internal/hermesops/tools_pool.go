...
func PoolListSpec(deps PoolListDeps) ToolSpec {
	return ToolSpec{
		...
		Description:  "分页列出当前租户的路由池及其策略、等待、超时、限流和启用状态；只读。",
		ReadOnly:     true,
		...
		Run: func(ctx context.Context, req ToolRequest) (ToolResult, error) {
			...
			rows, err := deps.List(ctx, dbbilling.ListPoolsParams{TenantID: req.TenantID, LimitCount: int32(limit + 1), PageOffset: int32(offset)})
			if err != nil {
				return ToolResult{}, err
			}
