// Package opsinspection 提供受管理员配置控制的每日运维巡检。
//
// 它按计划运行既有的只读诊断，从经过脱敏的系统诊断数据组合平台租户报告，
// 再通过既有邮件通道发送给已配置的管理员。所有工作都在请求热路径之外执行。
//
// 隐私边界：报告只携带系统诊断的枚举、计数和编号，绝不携带提示词、生成内容、
// 原始请求体、密钥、凭据、金额或个人信息。InspectionService 会再次投影为封闭字段，
// 上游新增的自由文本不会自动进入邮件正文。
