...
// ModuleSnapshotter 是巡检读取模块状态所需的最小只读接口。
type ModuleSnapshotter interface {
	Snapshot(ctx context.Context) []moduleregistry.ModuleSnapshot
}

// Sources 打包巡检复用的既有只读诊断函数，不重新实现查询逻辑：
//
//   - RenewStatus  == credentialstore.Store.ListRenewStatus，固定平台租户。
...
// NewInspectionService 构建该服务。tenantID 把渠道健康、凭据、用量和死信读取
// 全部限定到部署者平台租户。非正数租户不再猜成 1，而是拒绝构造。
...
// credentials 读取当前平台租户的续期状态，并统计其状态表明刷新失败或
...
// modules 读取模块知识快照并统计探针状态。探针报告 error 为 critical；
