...
func (w *InspectionWorker) Start(ctx context.Context) {
	w.mu.Lock()
	defer w.mu.Unlock()
	if w.started || w.svc == nil || w.sender == nil || w.recipient == "" || w.tenantID <= 0 {
		return
	}
...
func (w *InspectionWorker) runWithLeaderLease(ctx context.Context) {
	for {
		...
		if err != nil {
			w.failedTicks.Add(1)
			w.log.Warn("每日运维巡检主租约获取失败", zap.String("error_class", privacy.ErrorClassFor(ctx, err)))
...
func (w *InspectionWorker) runAsLeader(ctx context.Context, session workerlease.Session) {
	if session != nil {
		if err := session.Healthy(ctx); err != nil {
			w.failedTicks.Add(1)
			w.log.Warn("每日运维巡检主租约会话失效", zap.String("error_class", privacy.ErrorClassFor(ctx, err)))
			return
		}
	}
...
func (w *InspectionWorker) runClaimedTick(ctx context.Context) {
	if w.claim != nil {
		...
		if err != nil {
			w.failedTicks.Add(1)
			w.log.Warn("每日运维巡检时间窗占用失败", zap.String("error_class", privacy.ErrorClassFor(ctx, err)))
			return
		}
		if !acquired {
			return
...
func (w *InspectionWorker) tick(ctx context.Context) {
	...
	defer func() {
		if rec := recover(); rec != nil {
			w.failedTicks.Add(1)
			w.log.Error("每日运维巡检发生异常", zap.String("panic_class", "inspection_tick_panic"))
		}
	}()
	w.tickCount.Add(1)
	...
	if err != nil {
		...
		w.log.Warn("每日运维巡检邮件发送失败", zap.String("error_class", privacy.ErrorClassFor(ctx, err)))
	} else {
		w.reportsSent.Add(1)
		outcome.Sent = true
	}
