...
func (p *PASRSelector) Select(ctx context.Context, req SelectionRequest) (*SelectionResult, error) {
	...
	if len(prefixKey) == 0 {
		...
		return p.scheduleNoSegment(ctx, scoped, req, ring, snapshots, reason)
	}
	...
	if p.readOnlySegments {
		...
		if seg == nil {
			return p.scheduleHRWFullRing(ctx, scoped, req, ring, snapshots, [PASRSegmentSize]int64{}, selectionFailures{}, reason)
		}
	} else {
	...
	if len(candidates) == 0 {
		return p.scheduleHRWFullRing(ctx, scoped, req, ring, snapshots, seg.Members, failures, reason)
	}
