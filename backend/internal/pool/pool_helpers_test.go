...
func (s *stubAccountSource) ListAccounts(_ context.Context, req SelectionRequest) ([]*AccountSnapshot, error) {
	// 与生产 SQL 查询保持一致(sql/queries/pool_accounts.sql ListEligibleAccounts):
...
// captureClaimGate 记录 WriteAcquisition 调用。
...
// memSlotManager 发放 token；并跟踪释放次数。
