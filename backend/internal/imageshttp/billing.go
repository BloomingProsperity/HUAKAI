...
func (ex *execution) reserve(w http.ResponseWriter) bool {
	...
	ex.reserveRes = res
	if err := ex.settlementIntent.InsertPending(ex.ctx, ex.ident.TenantID, ex.requestID, ex.logicalRequestID, res.ClaimID, res.AttemptSeq, ex.ident.APIKeyID, ex.payloadHash, ex.predictedCost); err != nil {
...
func (ex *execution) settleDeliveredResponse(ctx context.Context, req billing.SettleRequest) error {
	if _, err := ex.d.Settler.Settle(ctx, req); err != nil {
		...
		evidence, enqueueErr := settlementrecovery.EnqueueFailure(ctx, ex.d.SettleRecoveryDLQ, payload, err, "imageshttp.settle_recovery")
...
func (ex *execution) abortAfterResponseWriteFailure(reason string, observedInputTokens int64, writeErr error) {
	...
	ex.settlementIntent.MarkAbortResult(ctx, abortErr)
...
func (ex *execution) abortWithLossError(w http.ResponseWriter, reason string, observedInputTokens int64, protocolLoss json.RawMessage) error {
	...
	defer cancel()
	err := ex.d.Settler.Abort(bctx, ex.ident.TenantID, ex.reserveRes.ClaimID, reason, ex.requestID, observedInputTokens, protocolLoss)
