...
func (ex *execution) prepareRoute(w http.ResponseWriter) bool {
	...
	if !modality.Supports(resolved.Capabilities, ex.endpoint.Modality()) {
...
func (ex *execution) screenInbound(w http.ResponseWriter) bool {
	protocol := moderation.ProtocolOpenAIAudioSpeech
	body := ex.body
	if ex.endpoint != audioEndpointSpeech {
		protocol = moderation.ProtocolOpenAIAudioTranscript
		body = audioTranscriptScreenBody(ex.req)
	}
