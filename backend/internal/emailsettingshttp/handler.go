...
func newAdminEmailSettingsPutHandler(d AdminEmailSettingsDeps) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		writeJSON(w, http.StatusOK, map[string]any{"tenant_id": req.TenantID, "updated": len(values)})
	}
}
...
func newAdminEmailTestHandler(d AdminEmailSettingsDeps) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			writeEmailConfigurationError(w, err, "email settings are not usable")
			return
		}
		...
		writeJSON(w, http.StatusOK, map[string]any{"tenant_id": req.TenantID, "sent": true})
	}
}
...
func writeEmailConfigurationError(w http.ResponseWriter, err error, message string) {
