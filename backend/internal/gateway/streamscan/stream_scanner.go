...
// Families 返回当前已登记的协议族副本，按字典序稳定排列。
// 它只用于启动装配校验与诊断，调用方不能借此修改注册表。
func (r *StaticStreamScannerRegistry) Families() []string {
	if r == nil || len(r.scanners) == 0 {
		return nil
	}
	families := make([]string, 0, len(r.scanners))
	for family := range r.scanners {
