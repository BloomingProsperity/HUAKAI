...
var requestMarshalers = map[string]func(*proto.HCSF) ([]byte, error){
...
// SupportsShape 报告归一后的请求形态是否具备编组器。
func SupportsShape(shape string) bool {
	_, ok := requestMarshalers[shape]
	return ok
}

// Marshal 把已归一的 HCSF 请求形态投影为上游请求体，且不静默丢弃能力。
func Marshal(env *proto.HCSF, shape string) ([]byte, error) {
	marshal, ok := requestMarshalers[shape]
	if !ok {
		return nil, fmt.Errorf("hcsf marshal: unsupported request shape %q", shape)
	}
	return marshal(env)
}
