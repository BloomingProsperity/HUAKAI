...
func TestTokenBucket_RefillAfter1Sec(t *testing.T) {
	...
	// 1s elapses → 2 tokens refilled (Rate=2)
...
func TestTokenBucket_RefillCapsAtBurst(t *testing.T) {
	...
	// Wait long enough that refill would exceed burst
	t1 := now.Add(10 * time.Second)
	tokens, _ := b.Snapshot()
	_ = tokens
	// trigger refill via Snapshot doesn't refill; use TryAcquire(0) trick
...
func TestTokenBucket_NextAvailableAt_Empty(t *testing.T) {
	...
	want := now.Add(500 * time.Millisecond) // 1 / 2 = 0.5s
	delta := next.Sub(want)
...
func TestTokenBucket_ConcurrentAcquireDoesNotExceedBurst(t *testing.T) {
	b := NewTokenBucket(0, 50) // rate 0 = no refill within test
	now := bucketTime()
	// First call sets the cursor. Use n=0 to stamp the time without consuming.
...
func TestTokenBucket_BackwardClockNoRegress(t *testing.T) {
	...
	_ = b.TryAcquireN(now.Add(-1*time.Second), 0) // backward time, no-op
	tokens, _ := b.Snapshot()
	if tokens != 3 {
