...
func (s *stubService) Create(_ context.Context, in routeadmin.CreateInput) (routeadmin.Route, error) {
	s.called = true
	s.lastCreate = in
	if s.createFn != nil {
...
func (s *stubService) List(_ context.Context, tenantID int64) ([]routeadmin.Route, error) {
	s.called = true
	if s.listFn != nil {
...
func (s *stubService) Get(_ context.Context, tenantID, id int64) (routeadmin.Route, error) {
	s.called = true
	if s.getFn != nil {
...
func (s *stubService) Delete(_ context.Context, tenantID, id, adminID int64) (routeadmin.Route, error) {
	s.called = true
...
func platformAdmin(tokenID int64) stubAuth {
	return stubAuth{ident: admin.AdminIdentity{TokenID: tokenID, Role: admin.RolePlatformAdmin}}
}

func newTestServer(d AdminDeps) *httptest.Server {
	r := chi.NewRouter()
...
func doReq(t *testing.T, ts *httptest.Server, method, path, body string) (int, map[string]any) {
	t.Helper()
...
func TestCreate_UsesAuthenticatedAdminIDNotBody(t *testing.T) {
	svc := &stubService{}
	ts := newTestServer(AdminDeps{Auth: platformAdmin(4242), Service: svc})
	defer ts.Close()

	body := `{"tenant_id":5,"name":"premium-claude","user_group_match":"premium","model_pattern_match":"claude-*","pool_group_id":9}`
	status, _ := doReq(t, ts, http.MethodPost, "/v1/admin/routes/", body)
...
// mutation: decodeJSON 去掉 DisallowUnknownFields() → admin_id 被静默忽略 → 201 + svc.called → 红。
...
// mutation: resolveAdmin 删掉 role==RolePlatformAdmin 检查 → 201 + svc.called=true → 红。
func TestCreate_NonPlatformAdminForbidden(t *testing.T) {
	svc := &stubService{}
	auth := stubAuth{ident: admin.AdminIdentity{TokenID: 9, Role: admin.RoleTenantOperator}}
	ts := newTestServer(AdminDeps{Auth: auth, Service: svc})
	defer ts.Close()
...
func TestAuth_ErrorMapping(t *testing.T) {
	...
	for _, c := range cases {
		svc := &stubService{}
		ts := newTestServer(AdminDeps{Auth: stubAuth{err: c.err}, Service: svc})
		status, _ := doReq(t, ts, http.MethodGet, "/v1/admin/routes/?tenant_id=5", "")
		ts.Close()
		if status != c.want {
...
// mutation: writeRouteError 任一分支落到 default → 该 case 变红。
func TestServiceError_StatusMapping(t *testing.T) {
	...
	for _, c := range cases {
		svc := &stubService{createFn: func(routeadmin.CreateInput) (routeadmin.Route, error) { return routeadmin.Route{}, c.err }}
		ts := newTestServer(AdminDeps{Auth: platformAdmin(1), Service: svc})
		body := `{"tenant_id":5,"name":"r1","user_group_match":"premium","model_pattern_match":"*","pool_group_id":9}`
		status, _ := doReq(t, ts, http.MethodPost, "/v1/admin/routes/", body)
		ts.Close()
		if status != c.want {
...
func TestGet_NotFound(t *testing.T) {
	svc := &stubService{getFn: func(int64, int64) (routeadmin.Route, error) { return routeadmin.Route{}, routeadmin.ErrRouteNotFound }}
	ts := newTestServer(AdminDeps{Auth: platformAdmin(1), Service: svc})
	defer ts.Close()
	status, _ := doReq(t, ts, http.MethodGet, "/v1/admin/routes/123?tenant_id=5", "")
...
func TestDelete_UsesAuthenticatedAdminID(t *testing.T) {
	svc := &stubService{}
	ts := newTestServer(AdminDeps{Auth: platformAdmin(7), Service: svc})
	defer ts.Close()
	status, _ := doReq(t, ts, http.MethodDelete, "/v1/admin/routes/55?tenant_id=5", "")
...
// mutation: parsePositiveQuery 放行空值 → service 以 0 调用 → 状态非 400 → 红。
func TestList_RequiresTenantID(t *testing.T) {
	svc := &stubService{}
	ts := newTestServer(AdminDeps{Auth: platformAdmin(1), Service: svc})
	defer ts.Close()
	status, _ := doReq(t, ts, http.MethodGet, "/v1/admin/routes/", "")
	if status != http.StatusBadRequest {
...
// 守依赖未配置: Service/Auth 为 nil → 503, 不 panic。
func TestNilDeps_ServiceUnavailable(t *testing.T) {
	ts := newTestServer(AdminDeps{Auth: nil, Service: nil})
	defer ts.Close()
	status, _ := doReq(t, ts, http.MethodGet, "/v1/admin/routes/?tenant_id=5", "")
