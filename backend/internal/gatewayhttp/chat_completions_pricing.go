...
func (ex *chatExecution) predictedCompletionCost() (decimal.Decimal, error) {
	return ex.completionCost(completionUsageForCost{
		InputTokens:  estimateInputTokens(ex.body),
		OutputTokens: estimateOutputTokens(ex.req),
	})
}

func (ex *chatExecution) actualCompletionCost(usage completionUsageForCost) (decimal.Decimal, error) {
	if usage.InputTokens <= 0 && usage.OutputTokens <= 0 && usage.CacheCreationTokens <= 0 && usage.CacheReadTokens <= 0 {
		return decimal.Zero, pricingUnavailable("reported usage missing")
	}
	return ex.completionCost(usage)
}

func (ex *chatExecution) completionCost(usage completionUsageForCost) (decimal.Decimal, error) {
	if ex.d.RateTables == nil {
		return decimal.Zero, pricingUnavailable("rate table source not configured")
	}
	version := strings.TrimSpace(ex.d.BillingPolicyVersion)
	if version == "" {
		return decimal.Zero, pricingUnavailable("billing policy version empty")
	}
	table, err := ex.d.RateTables.GetRateTable(ex.ctx, version)
	if err != nil {
		if errors.Is(err, billing.ErrRateTableNotFound) {
			return decimal.Zero, pricingUnavailable(fmt.Sprintf("rate table version %q not found", version))
		}
		return decimal.Zero, pricingUnavailable(fmt.Sprintf("rate table version %q read failed: %v", version, err))
	}
...
func (v completionRateVector) price(usage completionUsageForCost) (decimal.Decimal, error) {
	...
	totalMicros, err = addTokenBucket(totalMicros, usage.CacheCreationTokens, v.CacheCreation, v.HasCacheCreation, "cache_creation")
	if err != nil {
		return decimal.Zero, err
	}
	totalMicros, err = addTokenBucket(totalMicros, usage.CacheReadTokens, v.CacheRead, v.HasCacheRead, "cache_read")
	if err != nil {
		return decimal.Zero, err
	}
	if v.Multiplier.IsNegative() || v.Multiplier.IsZero() {
		return decimal.Zero, pricingUnavailable("rate vector model_multiplier must be positive")
	}
	return totalMicros.Mul(v.Multiplier).Div(decimal.NewFromInt(1_000_000)), nil
}
...
func addTokenBucket(total decimal.Decimal, tokens int, rate decimal.Decimal, hasRate bool, bucket string) (decimal.Decimal, error) {
	...
	return total.Add(decimal.NewFromInt(int64(tokens)).Mul(rate)), nil
}
...
func looksLikeRateVector(obj map[string]json.RawMessage) bool {
	for _, key := range []string{"input_micro_usd", "input_rate_micro", "output_micro_usd", "output_rate_micro"} {
