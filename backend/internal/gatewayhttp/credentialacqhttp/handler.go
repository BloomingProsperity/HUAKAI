...
type AdminCredentialAcquisitionDeps struct {
	...
	DeviceCodePoller         credentialacq.DeviceCodePoller
...
func startCredentialAcqFlow(w http.ResponseWriter, r *http.Request, d AdminCredentialAcquisitionDeps, ident admin.AdminIdentity, req credentialAcqStartRequest) {
	...
	if result.AuthType == credentialacq.AuthTypeDeviceCode || result.AuthType == credentialacq.AuthTypeSSO {
		resp["auth_type"] = result.AuthType
		resp["user_code"] = result.UserCode
		resp["verification_uri"] = result.VerificationURI
		resp["verification_uri_complete"] = result.VerificationURIComplete
		resp["poll_interval_seconds"] = result.PollIntervalSeconds
		resp["expires_in_seconds"] = result.ExpiresInSeconds
