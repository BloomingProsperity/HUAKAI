...
type codexAgentPlanRequest struct {
	TenantID int64                         `json:"tenant_id"`
	Content  string                        `json:"content"`
	Account  accountintake.AccountDefaults `json:"account"`
}

type codexAgentExecuteRequest struct {
	codexAgentPlanRequest
	PlanHash      string   `json:"plan_hash"`
	Confirmations []string `json:"confirmations,omitempty"`
	Reason        string   `json:"reason,omitempty"`
}

func newCodexAgentPlanHandler(d Deps) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		ident, ok := resolveAdminAccountIntake(w, r, d)
		if !ok {
			return
		}
		var req codexAgentPlanRequest
...
func newCodexAgentExecuteHandler(d Deps) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		ident, ok := resolveAdminAccountIntake(w, r, d)
		if !ok {
			return
		}
		var req codexAgentExecuteRequest
		if !decodeAccountIntakeJSON(w, r, &req) {
			return
		}
		if !validateAccountIntakeTenant(w, ident, req.TenantID) {
...
func (r codexAgentPlanRequest) planInput() accountintake.PlanInput {
	r.Account.AccountType = "session"
	return accountintake.PlanInput{
		TenantID: r.TenantID, SourceKind: intake.SourceCodexAgent,
		DefaultVendor: credentialstore.VendorOpenAI, DefaultAuthMode: credentialstore.AuthModeCodexAgent,
