...
func MountRoutes(r chi.Router, svc Service) {
	r.Post("/mj/submit/{action}", newSubmitHandler(svc))
	r.Post("/mj/insight-face/swap", newSwapHandler(svc))
	r.Get("/mj/task/{id}/fetch", newStatusHandler(svc))
	r.Get("/mj/task/{id}/image-seed", newStatusHandler(svc))
	r.Post("/mj/task/list-by-condition", newListHandler(svc))
}
...
func newSwapHandler(svc Service) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		ident, ok := requireSession(w, r)
		if !ok {
			return
...
func readListLimit(w http.ResponseWriter, r *http.Request) (int, bool) {
	limit := 100
	r.Body = http.MaxBytesReader(w, r.Body, maxRequestBodyBytes)
	raw, err := io.ReadAll(r.Body)
	if err != nil {
