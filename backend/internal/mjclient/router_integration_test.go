...
func TestMJSubmitFetchRoundTrip(t *testing.T) {
	...
	seed := seedMJUser(t, ctx, pool, "roundtrip")
	svc := newMJIntegrationService(pool)
	mux := mountMJIntegrationRouter(svc, seed)

	submitRec := httptest.NewRecorder()
	submitReq := httptest.NewRequest(http.MethodPost, "/mj/submit/imagine",
		strings.NewReader(`{"prompt":"round trip","botType":"MID_JOURNEY","notifyHook":"https://hook.example/rt"}`))
	...
	mux.ServeHTTP(fetchRec, httptest.NewRequest(http.MethodGet, fmt.Sprintf("/mj/task/%d/fetch", submit.TaskID), nil))
	...
	if fetched.ID != submit.TaskID || fetched.TaskType != "mj_imagine" || fetched.Provider != "midjourney" {
		t.Fatalf("fetched task id/type/provider=%d/%q/%q", fetched.ID, fetched.TaskType, fetched.Provider)
	}
}

func mountMJIntegrationRouter(svc Service, seed mjSeed) http.Handler {
	r := chi.NewRouter()
...
func openMJPool(t *testing.T, ctx context.Context) *pgxpool.Pool {
	t.Helper()
...
func seedMJUser(t *testing.T, ctx context.Context, pool *pgxpool.Pool, suffix string) mjSeed {
	t.Helper()
	var seed mjSeed
	name := fmt.Sprintf("mj-%s-%d", suffix, time.Now().UnixNano())
	...
	if err := pool.QueryRow(ctx,
		...
		seed.tenantID, seed.userID, "key-"+suffix, "$2a$10$mj-placeholder", "hk_test_mj_"+name,
...
func newMJIntegrationService(pool *pgxpool.Pool) *mediatask.Service {
	...
	return mediatask.NewService(store, mediatask.StaticConfigSource{Config: cfg}, mediatask.StaticProviderRegistry{
		"midjourney": mediatask.NewNoopProvider(),
	})
}
