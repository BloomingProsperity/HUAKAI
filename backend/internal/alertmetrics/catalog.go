...
var bridgedMeta = map[string]bridgedCatalogMeta{
	...
	"huakai_group_policy_failopen_total":           {"分组策略:fail-open放行", "次(累计)", "订阅分组策略因存储故障放行(fail-open)的累计次数;升高=分组限制正在被绕过。"},
	"huakai_group_policy_failclosed_total":         {"分组策略:fail-closed拒绝", "次(累计)", "订阅分组策略因存储故障拒绝(fail-closed)的累计次数;升高=用户被误拒。"},
