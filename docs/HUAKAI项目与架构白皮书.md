...
> 文档状态：现行主线总纲
> 事实基线：`origin/main@5edaa8856b2940e825042caac3a80662e232b634`
> 核实日期：2026-09-12
...
- 租户经营总览按租户服务端聚合 Token 分项、费用与日趋势；同一作用域另提供 UTC 小时桶，并列输入/输出/提示缓存写读 Token 与同窗费用，合计与序列同一窗口对账。同一租户同一窗口另提供业务缓存构成：上游提示缓存写/读 Token 与费用、L2 响应缓存命中次数与费用分栏，禁止合成；L2 草稿 Token 不得折进提示缓存栏。租户管理员只能看本租户；部署者必须显式指定租户，省略不得回落全平台。旧平台总览仍仅部署者。提示缓存 Token、响应缓存命中与 HTTP 快照缓存分列。
