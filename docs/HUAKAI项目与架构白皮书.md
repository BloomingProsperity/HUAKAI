...
> 文档状态：现行主线总纲
> 事实基线：`origin/main@8f58ca89985b44b0ea1c703f26d01e07b6b19fbe`
> 核实日期：2026-09-12
...
| `UP-001` | 规范请求与协议适配 | 将多入口请求归一化，并在上游和客户端协议间转换；思考块按出站账号族处理，翻译缺省输出上限取自模型目录；托管/检索档仅在能原样表达的协议族出站，否则拒绝。 |
| `UP-002` | 厂商 adapter | 封装厂商能力、端点、鉴权、模型和错误语义。 |
| `UP-003` | 出站与 TLS Sidecar | 选择标准 TLS、账号代理或特定仿真车道并执行网络出站。 |
| `UP-004` | 流式传递 | 解析、扫描、计量、透传和中断恢复流式响应。 |
| `MONEY-001` | 定价 | 解析模型、工具、图片、音频等价格并形成可版本化计价输入；部署者可按厂商模型覆盖绝对美元价，覆盖只影响新请求。已接线的完成态网页/文件/图像托管调用计入按次用量，未公布档不发明单价。 |
