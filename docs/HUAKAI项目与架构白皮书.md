...
> 文档状态：现行主线总纲
> 事实基线：`origin/main@c6fa08a1fca629587a283fb1e52f6dc76b489a10`
> 核实日期：2026-09-12
...
| Go 生产文件 | 1416 | 与 `docs/源码责任索引.md` 当前统计一致；排除 `_test.go` |
...
- 租户经营总览按租户服务端聚合 Token 分项、费用与日趋势。租户管理员只能看本租户；部署者必须显式指定租户，省略不得回落全平台。旧平台总览仍仅部署者。提示缓存 Token 与 HTTP 快照缓存分列。
...
白皮书负责稳定产品能力、架构边界和承重构件，不把 1416 个 Go 生产文件逐个复制进正文。
...
| `OPS-002` | 指标与用量分析 | 暴露运行指标、调用趋势、延迟、错误率和用量投影。租户经营总览按租户服务端聚合；部署者省略租户不得回落全平台。 |
