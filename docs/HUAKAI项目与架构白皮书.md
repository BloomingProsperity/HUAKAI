...
> 文档状态：现行主线总纲
> 事实基线：`origin/main@a30eaf15d00e3a283593834ad6fee43fd4418cbb`
> 核实日期：2026-09-12
...
- 同一租户作用域另提供按账号池的调度健康投影：每池把经未软删渠道挂入的未删账号分成互斥四栏 `schedulable / degraded / cooling_down / unavailable`（优先级 unavailable > cooling_down > degraded > schedulable），数据库层分栏谓词与生产选号候选查询、进程内健康门遵循相同门语义，再叠加本副本 auth 降级车道，只纳入与请求无关的账号级谓词，schedulable 表示健康层放行而非该请求必中；服务端给出计数、四位小数占比、池级状态（disabled > empty > unavailable > healthy > degraded）与最早已知恢复时刻（恢复时刻未知的冷却账号不参与，硬禁账号不参与），`unpooled_accounts` 单列用于对账：无并发变更时各池账号数之和加未挂池账号数等于租户级账号健康汇总总数。租户管理员锁本租户，部署者必须显式 `tenant_id`，真相不可用返回 503，不用空投影冒充全健康；响应不含账号标识与凭据元数据。已知限制：三条查询非同一快照事务；auth 车道叠加只反映本副本内存状态，车道持久化为后续路线项。
