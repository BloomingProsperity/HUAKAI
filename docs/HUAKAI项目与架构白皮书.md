...
> 文档状态：现行主线总纲
> 事实基线：`origin/main@5144a05698fb03995353e86fd7d27ec42d9b5706`
> 核实日期：2026-09-12
...
| Go 生产文件 | 1417 | 与 `docs/源码责任索引.md` 当前统计一致；排除 `_test.go` |
...
- 租户经营总览按租户服务端聚合 Token 分项、费用与日趋势；同一作用域另提供 UTC 小时桶，并列输入/输出/提示缓存写读 Token 与同窗费用，合计与序列同一窗口对账。租户管理员只能看本租户；部署者必须显式指定租户，省略不得回落全平台。旧平台总览仍仅部署者。提示缓存 Token 与 HTTP 快照缓存分列。
...
白皮书负责稳定产品能力、架构边界和承重构件，不把 1417 个 Go 生产文件逐个复制进正文。
...
| `OPS-002` | 指标与用量分析 | 暴露运行指标、调用趋势、延迟、错误率和用量投影。租户经营总览与小时 Token/提示缓存趋势按租户服务端聚合；部署者省略租户不得回落全平台。 |
