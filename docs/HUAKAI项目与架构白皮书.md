...
> 文档状态：现行主线总纲
> 事实基线：`origin/main@1d067efa051656034d87846866db4f0bd3875029`
> 核实日期：2026-09-13
...
| 资产 | 本提交实数 | 统计口径 |
| --- | ---: | --- |
| Go 包 | 357 | `backend` 模块内 `go list ./...` |
| Go 生产文件 | 1444 | 与 `docs/源码责任索引.md` 当前统计一致；排除 `_test.go` |
| 第一方 Rust 工作区源码 | 61 | 排除 `tests/` 与 `target/`；其中 9 个 Sidecar 文件进入生产镜像，其余不进入当前镜像 |
| PostgreSQL 迁移文件 | 466 | `backend/sql/migrations/*.sql` |
...
- 当前仓库不包含业务前端源码、静态页面或嵌入构建链，生产镜像按 API-only 构建。新前端尚未实现，
  页面规格以工程设计手册、OpenAPI 和生产源码为准。
...
- 当前部署是 API-only；新前端未实现前，页面草图和历史按钮不构成能力证据。
...
- 解析结果包含 canonical model、上游 model、协议族、能力、上下文窗口、候选 binding，以及目录登记的最大输出上限。
- 翻译到必须带 `max_tokens` 的上游且调用方未给正上限时，使用该目录正值且不得超过该值；未登记或非正则仍兜底 4096。官方直发与调用方已给上限不改。
...
- 已公布处理档按客户请求档预扣额度；`auto` 或省略按标准预扣。未公布档不按发明倍率预扣。
...
- 可续期凭据轮换时，若入站材料缺少或空置刷新令牌，必须保留库里已有续期材料；只有入站明确带来新刷新令牌才覆盖。官方 API Key 没有刷新生命周期，不走该合并。
- 上游个人身份、订阅、额度窗口、代理和 TLS 档案属于账号事实，不能散落到请求 body 临时猜测。

#### 第 8 段：协议和出站

- 客户协议先转成 HUAKAI 的 canonical/HCSF 表示，再由 Provider Adapter 生成上游请求。
- 思考块是否剥离无签名历史，只看出站账号所属厂商族，不信客户端自报的模型名。官方族可剥无签名思考；兼容族必须原样回传；未知族不剥。Chat 形可投影推理正文，但不回传签名。禁止伪造厂商思考签名。
- Messages 思考展示档只接受 `summarized`/`omitted`；`updates` 必须带官方 beta。未知档或关闭思考仍带 display 时本地拒绝，禁止改档或注入。缺省不覆盖模型默认。
- Responses/Codex 同协议出站时，官方托管/检索档原样携带，不得剥成只剩函数或改写成 function。未知档以及跨协议无法原样表达时 fail-closed，禁止剥了当成功。缓冲响应回传托管调用项与正文引用。
...
- 处理档只采信上游终态或带用量帧的实档，按实档只降不升结算；中间回声和请求信封不得当实档。未公布档不发明倍率，按 1× 暂记并标记待对账。给客户端的处理档字段只回实档或省略。重计价锁定已落账结算档。
- 已完成的网页检索、文件检索和图像生成调用计入按次用量；失败或进行中不计。未接线档不发明单价。
- 官方嵌套思考分解进入用量明细；金额仍按包容性输出计价，禁止把思考再加一笔。缺字段、明确 0 与空块且分解大于 0 必须可区分。
- 已向客户端写出任意业务字节后，即使后续刷新失败也按上游用量/次数结算并挂待对账；仅零业务字节且无上游成本才整笔释放预扣。禁止按已写字节比例发明价。
...
Chat、Responses、Messages 以及 Completions、Embeddings、Rerank、Images、Audio、Video、
count_tokens 与媒体任务提交的用户可控文本，在鉴权之后、预扣费或任务持久化之前进入同一
内容审核闸。多数同步入口在路由解析之后进闸；Video 与 Gemini countTokens 在模型解析之前
进闸，仍保证出站与扣费之前完成扫描。已登记但无用户文本（如图片变体、无提示转写）视为
已进闸。未知协议按租户失败策略关闭（默认关闭）。策略拒绝稳定 403。二进制音频/视频模型
级审核仍是强制路线。管理面 `/admin/v1/moderation/*` 已挂载，前端待接线。
...
| Codex 批量 | 解析多份凭据材料，复用统一计划、消歧、事务和结果合同。仅访问令牌材料在指纹未命中时仍按上游账号身份更新已有号，并保留已有刷新令牌 | `/account-imports/codex/*` |
...
运营可按选中账号 ID 批量执行启停、立刻刷新、清限流、完整恢复、改渠道与优先级/权重：字段与改渠道逐项独立短事务，刷新走有界工人池与后台准入链，部分成功不以整批成功语义返回；静态凭据不适用。
...
预测费用
  ...
  -> 上游调用
  -> 成功：按真实用量 Settle
  -> 已写出业务字节但交付中断：按上游用量结算并挂待对账，不按字节比例拆价
  -> 失败且零业务字节、无上游成本：Abort 释放
...
| 前端 | React + TypeScript + Vite（目标栈） | 当前仓库不包含业务前端；新前端独立构建并通过同源反向代理接入 Gateway，具体版本在初始化 PR 完成依赖审查后锁定 |
...
- 思考块按出站账号族保留或剥离；翻译路径缺 `max_tokens` 时按模型表正上限补值。
- 可续期账号导入更新若未带刷新令牌，轮换后仍保留库里已有续期材料。
- Chat 与 Completions 处理档按上游实档只降不升结算；未公布档不发明倍率并挂待对账。
- Responses 同协议保真官方托管/检索声明与正文引用；跨协议或未知档 fail-closed。完成态网页/文件/图像调用计入按次用量，不发明未公布单价。
- Messages 思考展示档原样出站或本地拒绝；官方思考分解写入用量明细且不双计。Chat 投影空思考块记显式损失。
- 已向客户端写出业务字节后的短写按上游用量结算并挂待对账；仅零字节且无上游成本才整笔释放。音频/图像/嵌入/补全/重排/非流 chat 同根。
- 租户经营总览按租户服务端聚合 Token 分项、费用与日趋势；同一作用域另提供 UTC 小时桶，并列输入/输出/提示缓存写读 Token 与同窗费用，合计与序列同一窗口对账。同一租户同一窗口另提供业务缓存构成：上游提示缓存写/读 Token 与费用、L2 响应缓存命中次数与费用分栏，禁止合成；L2 命中回放给客户端的输入/输出/提示缓存写读 Token 单列，不得折进提示缓存栏。totals 与小时趋势的 Token 是客户端实际收到的口径，包含 L2 回放 Token（费用 0），因此同窗 totals 的提示缓存 Token 恒等于上游栏加回放栏，运营面据此对账。租户管理员只能看本租户；部署者必须显式指定租户，省略不得回落全平台。旧平台总览仍仅部署者。提示缓存 Token、响应缓存命中与 HTTP 快照缓存分列。
- 同一租户作用域另提供按账号池的调度健康投影：每池把经未软删渠道挂入的未删账号分成互斥四栏 `schedulable / degraded / cooling_down / unavailable`（优先级 unavailable > cooling_down > degraded > schedulable），数据库层分栏谓词与生产选号候选查询、健康 FSM 门、auth 降级车道真相表遵循相同门语义并在同一条 SQL 内折算，只纳入与请求无关的账号级谓词，schedulable 表示健康层放行而非该请求必中；服务端给出计数、四位小数占比、车道改变栏位或恢复时刻的账号数（`auth_cooldown_accounts`）、池级状态（disabled > empty > unavailable > healthy > degraded）与最早已知恢复时刻（恢复时刻未知的冷却账号不参与，硬禁账号不参与），`unpooled_accounts` 单列用于对账：无并发变更时各池账号数之和加未挂池账号数等于租户级账号健康汇总总数。租户管理员锁本租户，部署者必须显式 `tenant_id`，真相不可用返回 503，不用空投影冒充全健康；响应不含账号标识与凭据元数据。已知限制：三条查询非同一快照事务。
- auth 降级车道以 `provider_account_auth_cooldowns` 表为跨副本唯一真相并默认开启（`HUAKAI_AUTH_COOLDOWN_ENABLED=false` 为逃生阀）：请求时鉴权失败按封顶指数退避临时移出选号，铁证类达阈值或凭据刷新拿到永久失效即硬禁；选号候选查询直读真相表（任一副本写入下一次候选查询即可见），进程镜像只承担本地即时性与写库失败时的兜底，每 5 秒重载并补写本地硬禁；硬禁只增不减，成功请求只清软退避，硬禁只由运营 resume（留下 `resumed_at` 墓碑，早于墓碑的本地硬禁补写被拒）或显式凭据轮换（同事务清行，旧凭据版本的结论不禁新凭据）解除；账号诊断给出 `persistence / failure_class / last_escalated_at / hard_disabled_at`，仅本副本持有的兜底状态另带 `replica_id`。已知限制：他副本刚写入软退避与本副本同时成功的竞态保留到软退避过期；墓碑比较依赖副本 NTP 同步；写库失败后崩溃的本地兜底丢失为有界自愈；逃生阀关闭后不再写入与 resume 清理，但候选查询仍排除存量硬禁与未过期软退避行，放行需凭据轮换或重新开启后 resume。
...
白皮书负责稳定产品能力、架构边界和承重构件，不把 1444 个 Go 生产文件逐个复制进正文。
...
| `ACC-002` | 账号添加与凭据导入 | 将 OAuth、Cookie、Setup Token、CLI 文件、批量文本或迁移包识别为账号并安全落库；仅访问令牌更新不得冲掉已有刷新令牌。 |
...
| `API-007` | Responses | 提供 OpenAI Responses 兼容入口；同协议原样出站官方托管/检索档并回传调用项与引用，跨协议或未知档拒绝。 |
| `API-008` | Anthropic Messages | 提供 Anthropic Messages 兼容入口；思考展示档原样出站或拒绝未知/无 beta 档，并回传嵌套思考分解。 |
| `API-009` | Gemini | 提供 Gemini 原生和兼容入口。 |
| `API-010` | 模型查询 | 返回当前身份、Key 和池组实际可调用的模型视图。 |
| `UP-001` | 规范请求与协议适配 | 将多入口请求归一化，并在上游和客户端协议间转换；思考块按出站账号族处理，翻译缺省输出上限取自模型目录；托管/检索档仅在能原样表达的协议族出站，否则拒绝；思考展示档不得改写或静默删除。 |
| `UP-002` | 厂商 adapter | 封装厂商能力、端点、鉴权、模型和错误语义。 |
| `UP-003` | 出站与 TLS Sidecar | 选择标准 TLS、账号代理或特定仿真车道并执行网络出站。 |
| `UP-004` | 流式传递 | 解析、扫描、计量、透传和中断恢复流式响应。 |
| `MONEY-001` | 定价 | 解析模型、工具、图片、音频等价格并形成可版本化计价输入；部署者可按厂商模型覆盖绝对美元价，覆盖只影响新请求。已接线的完成态网页/文件/图像托管调用计入按次用量，未公布档不发明单价。思考分解只进明细，不另加输出费。 |
| `MONEY-002` | 预扣、结算、退款与补账 | 在调用前保留额度，按真实用量和上游处理档实档结算，只降不升；未公布档不发明倍率。已写出业务字节后的交付中断按上游用量入账并挂待对账，禁止按字节比例拆价；仅零字节且无上游成本才整笔释放。多次补偿不重复记账。 |
...
| `OPS-002` | 指标与用量分析 | 暴露运行指标、调用趋势、延迟、错误率和用量投影。租户经营总览、小时 Token/提示缓存趋势与业务缓存构成按租户服务端聚合；部署者省略租户不得回落全平台。 |
...
每个目标只有一个 GitHub Issue 作为当前工作项。Issue 保存目标、当前状态、范围、决策、风险、
验收和未完成项；PR 保存真实差异、测试、审查和恢复证据。不得为同一目标再建第二份进度文档。
...
`阻塞`、`取消` 和 `转入事故` 是异常状态，必须在 Issue 说明原因、最后可信提交和恢复条件。
状态不能只写在聊天中。每次交接、暂停、恢复和关闭都更新同一个 Issue 或关联 PR，不新建
“交接报告”“当前进度”或 AI 会话记忆文件。

证据归属固定如下：

| 事实 | 唯一归属 |
| --- | --- |
| 当前目标、范围、状态、未完成项和 Owner 决策 | GitHub Issue |
| 修改文件、行为变化、影响半径、测试和审查 | 当前唯一 PR |
| 不可变补丁与提交说明 | commit |
| 自动验证结果 | CI |
| 现行产品、架构与稳定决策 | 白皮书和工程设计手册 |
| 已部署版本与发布结果 | Release |
| 重大生产事故的时间线、根因和纠正措施 | 事故 Issue 与最终 RCA |

### 15.4 多执行者交接

不固定 Codex、Claude、Grok 或其他工具的永久分工；Owner 当前指定谁，谁就是当前写入负责人。
同一时间仍只允许一个写入者。其他执行者只能在明确授权后只读调查、复现或独立审查。

交接前，原写入者必须在当前 Issue 或 PR 留下以下最小信息：

1. 分支、PR、最后可信提交 SHA 和相对最新 `origin/main` 的关系；
2. 已完成且有证据的事项；
3. 未完成项、阻塞项和仍待 Owner 决策的事项；
4. 已运行的命令、结果及尚未运行的验证；
5. 工作树是否干净；若不干净，逐项说明未提交文件归属；
6. 是否接触真实账号、秘密、资金、生产数据或外部副作用。

接手者必须重新 fetch，核对 Issue、PR、提交和 diff，并重跑与当前切片直接相关的最小验证，
不能仅凭上一段对话继续写。多个 AI 共用同一 GitHub 身份时，PR 中记录执行会话和复核会话；
同一账号下的文字“批准”不能伪装成平台可证明的独立人工审批。
...
正式事故只用于已经影响生产、发生真实跨租户/资金/秘密泄漏/数据丢失，或达到同等严重度的事件。
开发环境缺陷、未上线能力缺口和普通 CI 失败继续走缺陷 Issue，不能滥用事故文档。事故先在
同一个事故 Issue 维护时间线和处置；只有根因、影响、恢复与长期纠正措施核实后才形成一份最终
RCA，临时战报和重复复盘不得长期留在主线。
...
本表使用轻量 ADR 编号，但不再为每项决策复制一份 ADR 文件。`登记日` 是本次收束进唯一总纲的
日期，不冒充决策第一次提出的日期；原始讨论与变更历史由来源 Issue/PR 保存。

| ID | 状态 | 现行选择 | 直接后果 | 登记日 | 来源与替代关系 |
| --- | --- | --- | --- | --- | --- |
| `ADR-001` | 现行 | PostgreSQL 是权限、钱、凭据版本、租户、恢复和业务效果的权威事实 | Redis 和内存只能承载可重建短期状态 | 2026-07-24 | 当前主线 schema；由 [#302](https://github.com/BloomingProsperity/HUAKAI/issues/302) 与 [#299](https://github.com/BloomingProsperity/HUAKAI/pull/299) 统一登记 |
| `ADR-002` | 现行 | 发布前由一次性 migrate 任务升级，Gateway 不与多个实例竞争生产迁移 | 迁移失败阻止新版本接流量 | 2026-07-24 | 当前部署合同；由 #302/#299 统一登记 |
| `ADR-003` | 现行 | 只有部署者、下级租户管理员、最终用户三种人类身份 | “代理”等称呼不能生成第四套权限模型 | 2026-07-24 | Owner 现行指令；替代历史“下级代理”并行角色表述 |
| `ADR-004` | 现行 | 部署者调下级租户钱包，租户管理员调本租户用户；部署者不得越级调下级租户用户 | 所有支付、订阅、兑换和人工调整入口必须复用该边界 | 2026-07-24 | Owner 现行指令与当前账本合同；由 #302/#299 统一登记 |
| `ADR-005` | 现行 | 用户 Key 经模型目录和池组 binding 间接取得上游账号 | Key 不直接持有上游凭据 | 2026-07-24 | 当前数据面主链；由 #302/#299 统一登记 |
| `ADR-006` | 现行 | 官方 Key 与普通兼容上游走标准 TLS；明确需要客户端形态的账号模式走 Rust mimicry；CRS 安全源传输当前也复用 Sidecar | 统一 Dispatcher 不等于所有推理流量统一进 Sidecar，Sidecar 当前仍是全局启动与就绪依赖 | 2026-07-24 | Owner 现行指令与当前出站接线；替代“所有账号统一走 Rust”旧口径 |
| `ADR-007` | 现行 | 管理 CRUD 必须成为热路径真实读取的事实 | 只有字段、按钮或表但无消费者视为空心能力 | 2026-07-24 | Owner 现行指令与完整链路规则；由 #302/#299 统一登记 |
| `ADR-008` | 现行 | 产品统一称日志系统；普通日志保留 30 天，资金业务事实永久保留 | 技术包名可保留，运营口径不能再分裂 | 2026-07-24 | Owner 现行指令；替代“日志/审计系统”并列产品口径 |
| `ADR-009` | 现行 | 使用官方 Hermes Runner；HUAKAI 只提供租户隔离的工具、确认、限制和恢复 | 最终用户无入口，Runner 无数据库和宿主访问 | 2026-07-24 | Owner 现行指令与当前 Hermes 接线；由 #302/#299 统一登记 |
| `ADR-010` | 现行 | 退役旧业务前端、静态嵌入和 SPA 回退，新前端按后端真实能力独立建设 | Gateway 保持 API-only；后端状态不能因页面尚未实现被误判不存在 | 2026-07-28 | Owner 现行指令；替代从旧页面恢复实现的口径 |
| `ADR-011` | 现行 | 最新主线建功能分支，单目标、单写入者、单 Issue、单 PR，Owner 批准后合并 | 历史计划、聊天记忆和旧 PR 不再形成平行真相 | 2026-07-24 | Owner 现行指令与 [#302](https://github.com/BloomingProsperity/HUAKAI/issues/302)；替代并行双计划制度 |
...
包总数不是首要问题。357 个 Go 包中，大量是按 HTTP、领域、Provider、协议和 sqlc 查询边界
