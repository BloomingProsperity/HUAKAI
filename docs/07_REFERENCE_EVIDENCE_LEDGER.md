...
| E-S2A-005 | Sub2API (E-LIC-001) | Public README | A `RUN_MODE=simple` flag hides all SaaS features (payment, multi-user billing) for internal-team deployments. | Edition-flag plumbing that toggles SaaS-only features off (DR-002 Personal vs SaaS in concrete form). | Simple Mode users may run without payment guardrails and over-consume; need cost ceiling alert anyway. | Behavior only. | 2026-04-28 | Claude (specifier) |
