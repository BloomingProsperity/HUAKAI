...
- 修补措施：已装 `.git/hooks/pre-push`，从 L2-A5.5 起每个 slice push 前必须有 `docs/process/reviews/` 下匹配 slice id 的 review artifact。
