...
## GW-WIRE-004 代码注释 clean-room 收口批

| 项目 | 内容 |
| --- | --- |
| Scope | 枚举 `backend/**/*.go` 中直接出现参考项目名或历史 `SUB2-*` 标签的代码注释，逐处改写为 HUAKAI 自身的机制、风险、不变量和测试意图；生成 SQL Go 文件对应的查询源注释同步修正。字符串协议值、第三方依赖名、许可证、行为实现和测试断言不在本批修改范围。 |
| Success criteria | `.go` 注释不再出现参考项目名或 `SUB2-*` 标签；每条注释仍能独立解释 HUAKAI 为什么这样做；生成文件不会因重生成恢复旧注释；全仓单测、`go vet` 和质量门通过；差异中不存在行为代码变化。 |
| Blast radius | 仅注释和内部说明标签；若误改字符串或生成源不同步，可能造成行为变化或后续重生成漂移。使用差异审查、Go 词法级注释扫描和全仓测试控制。 |
| Failure modes | 把普通英文短语误判成项目名；只改生成 `.go` 不改 SQL 源；机械删除后留下无法理解的半句；把参考行为结论继续藏在生产注释中。逐文件打开上下文后改写，不按搜索结果盲删。 |
| Decision points | 本批不改变 GW-WIRE-004 网页 session 能力的注册、合规状态或 Mandatory Roadmap 处置；只闭环已确认的代码注释硬规则缺口，不需要新增 Owner 决策。 |
