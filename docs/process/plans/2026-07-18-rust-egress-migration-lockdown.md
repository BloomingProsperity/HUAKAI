...
| 项目 | 内容 |
| --- | --- |
| Owner directive | “审计系统统称为日志”；“日志为 1 个月为周期，超过自动删除”；“日志系统要详细，分类明确，错误要标记清楚”；“日志 30 天清理属于全局生效，分类也对全局生效”。 |
| 范围 | 平台统一日志信封、slog/zap 采集、访问日志、运行日志查询、自动与人工清理、健康状态、启动与停机顺序、OpenAPI 和 PostgreSQL 迁移；横向登记全部普通日志表。 |
| 不在删除范围 | 资金账本、余额/充值/退款/争议效果、幂等与去重事实、定价哈希链、处罚实时状态、Outbox/DLQ/待恢复任务、签名密钥和清理自身状态。它们虽然部分历史命名含 `log/audit/event`，但属于业务真相或恢复依据，不能按日志删除。 |
| 成功标准 | 所有新入统一运营面的日志均有稳定类别、事件类型、结果、错误分类/码和关联字段；显式分类的 Info 与全部 Warn/Error 可入库；普通日志严格按可信入库时间保留 30 天；启动补扫、周期清理、人工清理共用同一策略；单批有界、多副本不重复工作、失败可见；不存在任意时间点或全表清空入口。 |
| 爆炸半径 | 误删会破坏资金、幂等、处罚或恢复依据；Info 洪峰会挤掉异常；迁移先后错误会让启动期日志持续丢失；多副本无协调会放大数据库负载。 |
| 失败模式与缓解 | 仅用明确表白名单；每表固定可信时间列；事务级租约 + 小批删除；Info 只有带完整分类合同才采集，异常使用独立优先队列；迁移早于 sink/清理器启动；健康面公开积压、丢弃、最近尝试/成功和失败分类。 |
| 决策点 | 无。Owner 已锁定平台全局分类和 30 天自动删除；当前 PostgreSQL 无列式日志库，也无需要长期保留的法规合规库。未来若出现法律保全，必须新建独立合规存储，不能悄悄改写普通日志合同。 |

源码对照形成的行为合同：

- 稳定机器维度与展示文本分离；分类采用 `operation/financial/security/error/access/recovery`，事件类型不使用自由文本，结果与错误分类明确可筛选。
- Info 只有显式携带有效分类与事件类型时才进入统一运营面；Warn/Error 无条件保留最小诊断事件并在缺字段时安全归入错误类别，访问洪峰不能挤掉异常。
- 固定截止线由 PostgreSQL 可信时钟一次生成，删除条件为 `ingested_at < 数据库当前时间 - 30 天`；事件自报时间仅供展示，不能延长或缩短实际保留期。
- 自动清理启动后错峰补扫并周期执行；每次事务只删固定小批、总轮次和总时限有界；多副本使用 PostgreSQL 事务级租约，同一时刻只有一个执行者。
- 人工清理只触发相同固定策略，不能接收自定义 cutoff；必须先写强一致管理操作日志，该记录也按全局 30 天合同保留，不冒充永久资金事实。
- 清理白名单逐表核实源码用途，不按表名猜测。普通运营日志表可以删除；会驱动余额、幂等、处罚、恢复或签名验证的事实表永久排除。

执行顺序与判别门：

1. [x] 读取 HUAKAI 真码，逐表判定普通日志与持久事实；读取中转站三镜及日志领域项目源码，clean-room 提炼行为合同。
2. [x] 新迁移扩展统一日志信封，增加可信入库时间、稳定分类与查询索引；开发回退把旧版无法表示的 Info 映射为 warn，保留全部日志行，只移除 0195 新增元数据。
3. [x] 建立独立日志合同包；改造 slog/zap 为异常优先双队列，显式分类 Info 入库，字段统一脱敏和规范化；HTTP `4xx` 保留失败字段但走 Info 容量，只有 `5xx` 占用服务端异常容量。
4. [x] 建立固定 30 天全局保留器：表白名单、有界批次、事务级租约、启动补扫、周期执行、健康状态和结构化恢复日志。
5. [x] 收紧管理员接口与 OpenAPI：增加分类筛选，删除任意 cutoff，人工触发同一策略并保留不可清理的管理操作事实。
6. [x] 调整启动/停机顺序；数据库迁移先于日志 sink，保留器先停、sink 排空后再关数据库。
7. [x] 判别测试覆盖分类 Info/异常优先、字段脱敏、30 天边界、全部白名单、永久排除表、批次上限、多副本租约、人工越界拒绝、迁移往返和故障健康。
8. [x] 运行受影响包 race、全量 `make test`、vet、PostgreSQL 集成、migration 往返、`git diff --check` 和两轮独立只读复审；所有审查发现均已修复，无遗留 S0/S1，单独提交到现有 PR 分支。

验证结果（2026-07-18）：

- [x] 全量普通测试与竞态门通过；全新 PostgreSQL 从 `template0` 迁移 1 -> 195 后，全部 `integration_pg` 包以 `-p 1 -race -count=1` 通过。
- [x] `make vet`、`make quality-gate`、`go mod tidy -diff`、严格 OpenAPI YAML 重复键检查和 `git diff --check` 通过。
- [x] 存量回填判别通过：迁移前 90 天旧事件在 0195 迁移后保留，可信 `ingested_at` 从迁移时刻起算，不会立即被清理。
- [x] 0195 -> 0194 -> 0195 真库往返通过；旧版无法表示的 `Info` 在回退时映射为 `Warn`，全部原日志行保留。
- [x] 独立复审第一轮发现回退删除 `Info` 行，工具标为 P2，按 HUAKAI 数据丢失规则归一为 S1 后修复；第二轮发现 `4xx` 与 `5xx` 共用异常容量，按生产可观测性保障归一为 S1 后修复。按两轮上限停止复审，新增级别隔离测试及 access/logsink 竞态测试通过，无遗留 S0/S1。
