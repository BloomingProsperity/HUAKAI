# 2026-05-22 Rust 网关加固计划 rev2 —— W11 安全边界 + W12 账务遥测 + 指纹（Claude 稿）

> **Rust 并行线**（独立于 Go 线 W4…）。本会话 = clean-room **specifier 车道**。
> **状态：rev3 — 处理 Codex 对 rev2 的再次 REJECT（1 HIGH D-4 时序架构 + 2 MED：D-5 bad-JSON 测试、D-6 控制面注入 openai-project 保留断言），待 Owner 审批 + Codex 再评审。** 审批前不写任何实现代码。
> rev1 → rev2 变更：D-1 身份设计闭合（拆 D-1a/D-1b + 三选项 + Owner 决策点）+ D-4 已定 C2 durable spool（5 条 acceptance criteria）+ D-5 复用 `tokens_used.source` 词表（无 proto 改）+ 清理 clean-room 标识符泄漏 + 强化 D-3/D-6/D-7/D-8/F-1/F-2/O-2 判别性测试 + 增 §4.5 route.proto 受控变更集 + 新增 3 个 Rust 参考（clewdr/smg/litellm-rs）。
> **rev2 → rev3 变更**：D-4 修法增"pre-commit gate vs post-commit drop"时序拆分（响应头一旦送出 HTTP 不可逆，rev2 旧 AC-4 的"返回 5xx"对 post-commit 不成立——架构现实修正）+ AC-4 拆为 AC-4-pre/AC-4-post + AC-5 加 watermark 触发 pre-commit gate；D-5 测试加 T5（结构不完整 JSON 走 `pending_reconciliation`）；D-6 测试 T2 同时断言注入的 `openai-organization` 与 `openai-project` 保留。Codex rev3 评审 **APPROVE WITH MINOR**，仅文案修复（AC 5 条 → 6 条 stale）。
>
> **rev3.1 Owner 决策**（2026-05-23）：**D-1b = β（控制面权威身份）已定**；§4.5 **P-1 已批**（D-1b=β 强制要求）、**P-2 已批**（rate_limit_reset 纳入本轮）、**P-3 已否定**（不改 heartbeat 为 optional，转文档化"未接源"）；§7-A/B/E 推荐项默认通过（波次顺序 W11→W12→指纹、D-3 默认安全、L2 HTTP/2 走 L2-α）；下一阶段：Codex 平行起草 `-codex.md`（按 CLAUDE.md #10 严格平行模式，互不见）+ 双稿对比合成定稿。**§7 G/H 全部已决；W11 全部 finding 均可立即开干。**
...
**参考做法（多源一致）**：clewdr 上游账号选择全服务端、model 取自请求体 `model` 字段（`clewdr@57626809:src/services/cookie_actor.rs:176-199`，`clewdr@57626809:src/claude_code_state/chat.rs:55`）；litellm-rs 路由从已解析 JSON 请求体取 model 名走服务端路由表选择（`litellm-rs@82d01816:src/server/routes/ai/chat.rs:281-308`）；sub2api 路由输入只从 body 取且非字符串 model 严格 400（`Wei-Shaw/sub2api@f59d9a5f8e0983f4a92c1ba8c4ecd8d33f7e4427:backend/internal/service/gateway_request.go:151-166`）。**三源一致：model/stream 取自 body，不取自 header。**
...
**参考做法（多源，smg 为金本位）**：
- smg 在中间件按严格优先级解析租户身份（`smg@9a93938a:model_gateway/src/middleware/tenant_resolution.rs:58-73`）：① bearer auth 中间件已存放的"已认证调用方"对象赢；② **显式 opt-in 的"可信内部 header"模式**（默认 OFF）；③ 客户端 socket IP 兜底；④ 匿名 sentinel。身份对象在 request extensions 传递，下游处理器从那里读，**不再从 wire 重读**（`smg@9a93938a:model_gateway/src/middleware/tenant_resolution.rs:121-139`，`smg@9a93938a:model_gateway/src/tenant.rs:90-133`）。Bearer token 经 SHA-256 常时比较，租户键**由 hash digest 派生**——客户端无法自选键（`smg@9a93938a:model_gateway/src/middleware/auth.rs:35-66`，`smg@9a93938a:model_gateway/src/tenant.rs:154-164`）。身份来源类型保留在 enum（authenticated/header/IP/anonymous，`smg@9a93938a:model_gateway/src/tenant.rs:11-32`）。
- clewdr：账号选择从不读客户端 header/body 决定（`clewdr@57626809:src/services/cookie_actor.rs:176-199`）。单租户，无 tenant 抽象。
...
**推荐组合**：β 为默认，γ 作为受信前置层下的 opt-in 兜底（默认 OFF）。`x-tenant-id` 在 γ 关闭时**忽略**；在 γ 开启时仍需前置层验证才生效。
...
**判别性测试**：
- T1：凭据解析为 tenant A + 伪造 `x-tenant-id: B`，trusted-header opt-in **默认关** → 断言查询/计划的 tenant 为 A，**永远不是 B**。Mutation：回读 header → 红。
- T2：trusted-header opt-in **开** + 请求**不来自受信前置层** → 断言 `x-tenant-id` 仍被忽略。
- T3：trusted-header opt-in **开** + 请求**来自受信前置层** + `x-tenant-id` 存在 → 断言生效，身份来源 enum 标 `header` 而非 `authenticated`，控制面/审计可区分。
- fixture 关键：A 与 B 必须为两个**显式不同**租户值（不能用 "default-tenant" 这种默认值，会被默认逻辑掩盖）。
...
**参考做法**：sub2api 数据面无任何运行期 mock/test 旁路——上游 transport 是 interface，fake 只在 `*_test.go` 由构造器注入，无 env/config 开关在请求期换 fake（`Wei-Shaw/sub2api@f59d9a5f8e0983f4a92c1ba8c4ecd8d33f7e4427:backend/internal/service/account_test_service.go:173-321`）。clewdr 行为同向：所有安全相关旋钮是运行期配置 + 默认 fail-closed，cargo features 仅控资源打包不静默改 auth/transport 安全（`clewdr@57626809:src/config/clewdr_config.rs:103-110`，`clewdr@57626809:src/config/constants.rs:138-140`）。
...
- sub2api：两级 validator 但**默认不安全**——URL 白名单默认关、允许私网、允许明文 http；白名单关时只跑 format validator。定义了 resolved-IP validator 防 DNS-rebinding 但**转发路径未接线**。
- clewdr：上游基底是硬编码 https 常量（`clewdr@57626809:src/config/constants.rs:14`），**但 operator 反代覆盖无 scheme 检查**（`clewdr@57626809:src/config/clewdr_config.rs:377-382`）——可被运维错配引入明文；不程式化强制 https。
- smg：worker URL 校验放行 `http`/`https`/`grpc`/`grpcs` 四选一白名单，明文通过（`smg@9a93938a:model_gateway/src/config/validation.rs:828-873`）。"反例"参考。
- litellm-rs：有 SSRF guard 拒绝非 http(s)/ws(s) + 阻断私网/loopback/metadata（`litellm-rs@82d01816:src/core/net/ssrf_guard.rs:39-99`）；**但允许明文 http**，且 guard **未接到 provider 请求路径**——provider client 直接 `reqwest` 凭配置 base URL 调（`litellm-rs@82d01816:src/core/providers/openai/client.rs:116`）。**"guard 存在但不在转发路径上"的典型陷阱。**
...
- clewdr：**根本不转发客户端 header 集**——重建请求、服务端设 header，上游 auth 永远服务端铸（`clewdr@57626809:src/claude_code_state/chat.rs:128-144`）；只回显 `anthropic-beta`（合并）。drop-by-omission 的结构性保证。
- smg：内部路径 allowlist 显式**阻断 `x-api-key` 透传**（测试断言），上游 auth 按 provider 服务端铸；错误响应体里 upstream org/project 标识被正则**擦除**后回（`smg@9a93938a:model_gateway/src/routers/common/header_utils.rs:289-331`，`smg@9a93938a:model_gateway/src/routers/error.rs:161-203`）。
- litellm-rs：上游 auth 从 provider 配置服务端铸，org/project header 来自 config 而非客户端（`litellm-rs@82d01816:src/core/providers/openai/client.rs:48-69`）。
...
**参考做法**：HUAKAI 内部逻辑顺序 bug，参考项目无直接对应模块（cliproxyapi 用固定 utls preset，无此分级 gate）。**原则佐证**——clewdr 安全相关行为由运行期配置门控且 fail-closed，cargo features 不被设计为"静默改安全行为"的载体（`clewdr@57626809:src/config/clewdr_config.rs:103-110`，`clewdr@57626809:src/config/constants.rs:138-140`）。D-10 恰是反例：build feature 绕过运行期 gate。
...
- sub2api：响应后 best-effort 投有界 worker 池，**溢出时按采样策略丢弃**，无 durable spool；进程崩溃整个内存队列全丢（核验已确认，rev1 此处的上游字段标识已替换为行为描述）。
- clewdr：用量持久化是**内存计数 + 周期快照**（计数活在内存，整 config 定期 TOML 落盘）；**无 per-request durable ledger**，两次快照之间崩溃丢最近增量（`clewdr@57626809:src/services/cookie_actor.rs:58-79`）。
- smg：用量只作进程内 Prometheus 风格计数器；**无 durable ledger、WAL、可生还账记录**；最 best-effort。
- litellm-rs：**最差**——budget tracker 是内存 DashMap、无持久化，崩溃丢全部 spend（`litellm-rs@82d01816:src/core/budget/tracker.rs:16-21`）；log aggregator 内存 Vec，flush 失败时条目已 drain → **丢而不重试**（`litellm-rs@82d01816:src/core/observability/logging.rs:57-72`）；全仓库无 spool/outbox/dead-letter/idempotency。
...
- clewdr：读完非流式 body、解 `usage` 抽 input/output token；**缺时回退用本地 tokenize 估算 output**（`clewdr@57626809:src/claude_code_state/chat.rs:403-452`）。估算不可审计、可能错。
- smg：Anthropic 路径反序列化非流式 body 抽 token + 记 metric（`smg@9a93938a:model_gateway/src/routers/anthropic/worker.rs:88-108`）；**但 OpenAI 兼容代理路径原样字节转发不解析**（`smg@9a93938a:model_gateway/src/routers/openai/chat.rs:222-231`）——跨路径不一致。
- litellm-rs：每 provider transformer 各自解 usage；OpenAI 类型化路径缺 usage 时给 `None`（无错）；**Anthropic 手解 Value tree 用 get-then-coerce + 默认 0**——一个 numeric 字段缺失/非数字被**静默计费为零，无任何标记**（`litellm-rs@82d01816:src/core/providers/anthropic/client.rs:730-767`）；**全仓库无 reconciliation 机制**；**非流式无 body size cap**（`litellm-rs@82d01816:src/core/providers/openai/client.rs:120-142`）；OpenAI 客户端**解析前不检 HTTP status**——错误响应可能被当成 success 解析。
...
- T3 缺 usage：非流式 200 body **无 `usage` 对象** → 断言 `source="pending_reconciliation"`，**非** `"missing"`，**非** tokens 0 作终值。Mutation：静默计零 → `source` 会是 `"missing"` 或 tokens 0 → 红。**这是 litellm-rs `.unwrap_or(0)` 缺陷的精确镜像**——fixture **必须**是 present-but-incomplete body，让"只验 happy 路径"的弱测试无法通过。
...
**参考做法**：clewdr 无 node health（单进程，无多节点概念）；smg 暴露**真实测量**信号——in-flight tracker（live map keyed by id + start timestamps + age histogram，驱动 graceful drain，`smg@9a93938a:model_gateway/src/observability/inflight_tracker.rs:22-124`）+ per-worker 原子负载计数器由 **RAII guard tied to response body lifetime**（流式请求计到 stream 结束，`smg@9a93938a:model_gateway/src/worker/worker.rs:638-654`）。smg **不**做 node 级 queue-depth/computed error-rate/latency-percentile 作健康信号——HUAKAI 的 headroom。
...
**参考做法（四源融合）**：
- clewdr：429 分两子类（long-context-usage 429 留 plain、其他 429 → rate-limit reason 携 reset 时间戳——从 body 解、再 fallback 响应 header、再 fallback 1h，`clewdr@57626809:src/error.rs:364-403`）；**无 408 处理、无 `Retry-After` 解析**。
- smg：固定 retryable 集 {408, 429, 500, 502, 503, 504}，401/403 不重试；传输层失败映射到合成 status（`smg@9a93938a:model_gateway/src/routers/common/retry.rs:10-20`）；**无 `Retry-After`/rate-limit-reset 解析**——纯指数退避忽略上游时序。
- litellm-rs：429→rate-limit→retryable，408/504→timeout→retryable，401/403→auth→不重试；显式 retryable/non-retryable 分离（`litellm-rs@82d01816:src/core/router/execution.rs:16-24,58-86`）；Anthropic 客户端从 **429 响应体**解 retry-after（关键字 60s fallback，`litellm-rs@82d01816:src/core/providers/shared.rs:209-231`）；**全 provider 都不解 `Retry-After` HTTP header**——退避忽略上游时序。
...
- smg：代理路径**不数响应字节**。
- litellm-rs：字节计量**极简、信 header**——请求大小**只从入站 `content-length` header**取、缺失则 0（`litellm-rs@82d01816:src/server/middleware/metrics.rs:77-82`）；响应字节**完全不数**（`litellm-rs@82d01816:src/server/middleware/metrics.rs:87-103`）；analytics 的 request-size 字段**全仓库只在 `#[cfg(test)]` 块**赋值（`litellm-rs@82d01816:src/server/types.rs:219-431`）。**litellm-rs 与 HUAKAI 缺陷同形**——Rust 网关的 D-9。
...
**修法**：采 smg 的 **RAII guard idiom**——连接建立时 inc、连接/响应生命期结束（drop）时 dec；流式连接的 RAII guard 把"未结束流"保留在计数里。引用：`smg@9a93938a:model_gateway/src/worker/worker.rs:638-654`。
...
**Go 参考都不做 HTTP/2 指纹。** cliproxyapi 用 utls 造 TLS ClientHello 但 HTTP/2 用 Go 原生 stack；sub2api 造 ClientHello 但 `ForceAttemptHTTP2` 被关、ALPN 默认 `http/1.1`（`Wei-Shaw/sub2api@f59d9a5f8e0983f4a92c1ba8c4ecd8d33f7e4427:backend/internal/pkg/tlsfingerprint/dialer.go:361-364`）。**Rust 参考亦都不做指纹**——clewdr/smg/litellm-rs 都是"诚实客户端"。→ **L2 HTTP/2 接线是 HUAKAI 领先所有参考的真实价值点**，且 HUAKAI 的 adapter 已存在，主要是接线工程。
...
|---|---|---|---|---|---|
| **P-1** | **已批（Owner 2026-05-23）** | D-1b | `RouteQueryRequest` 增 `client_credential`（`bytes` 或 `string`）；或改走 gRPC request metadata 携带 | D-1b = β 已定 | Rust 添字段 + Go 控制面认证消费（**需 Owner 在 Go 线启动 spec**） |
