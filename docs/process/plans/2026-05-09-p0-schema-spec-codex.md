...
**对应 Claude lane**: docs/process/plans/2026-05-09-p0-schema-spec-claude.md（写作时未见）
...
P-0b 不入库，不改 billing ledger / auth core / quota enforcement / DB schema。HCSF v0.4 仅是内存 IR，native passthrough 采用 standard auth + audit 的 schema 表达，不在本阶段实现路由。依据：Owner 已批 D1/D3 不入库，D5 standard auth + audit，且合成计划把 P-0 输出限定为 Go type/JSON 字段锁定（`docs/process/plans/2026-05-09-hcsf-v04-implementation-synthesis.md:127-145`）。

## 1. HCSFEnvelope v0.4 顶层结构 (Go + JSON)

### 1.1 顶层锁定

P-0b 在 `backend/internal/proto` 内新增以下类型。代码注释必须中文，标识符与 JSON tag 保持英文；本仓既有计划已记录“注释中文、标识符英文”的 Owner 约束（`docs/process/plans/2026-05-08-pasr-lite-v2-codex.md:154`, `docs/process/plans/2026-05-09-next-pivot-claude.md:228`）。我没有在仓库中定位到独立的 `feedback_chinese_comments.md` 文件，风险见末尾。
...
`CapabilityResults` 的状态枚举直接锁定 P-5 matrix cell 的五态：supported/lossy/native_required/unsupported_error/roadmap。该五态来自 Codex lane 的 capability matrix strategy，用来保证每个 vendor/capability cell 不会 silent drop（`docs/process/plans/2026-05-09-hcsf-v04-implementation-codex.md:253-257`, `docs/process/plans/2026-05-09-hcsf-v04-implementation-codex.md:267-277`）。Tier-A projection 必须覆盖 Anthropic、OpenAI Chat、OpenAI Responses、Gemini、Bedrock-on-Anthropic；D11 决定 OpenAI Responses 只经 `/v1/native/openai/responses` 暴露，不开 public `/v1/responses`（`docs/process/plans/2026-05-09-hcsf-v04-implementation-synthesis.md:70-80`, `docs/process/plans/2026-05-09-hcsf-v04-implementation-synthesis.md:156-159`）。
...
`Policy.Auth` 只允许 `standard`，因为 D5 已批 native passthrough 使用 standard route auth 并增加 audit；P-0b 不实现 auth core 变化（`docs/process/plans/2026-05-09-hcsf-v04-implementation-synthesis.md:142-145`）。`DataRetention.Value` 锁定 D12 五词汇，`zdr_verified` 必须有 Owner/vendor/account proof，不能从 generic request field 推断（`docs/process/plans/2026-05-09-hcsf-v04-implementation-synthesis.md:161-164`, `docs/process/plans/2026-05-09-hcsf-v04-implementation-codex.md:344-345`, `docs/process/plans/2026-05-09-hcsf-v04-implementation-codex.md:362-368`）。
...
该测试属于 P-0 schema shape test。Codex lane 已要求 P-0 输出 compile-level schema JSON shape、loss severity validation、text-only canonical event round-trip 稳定性；P-5 再扩大为 capability/property matrix（`docs/process/plans/2026-05-09-hcsf-v04-implementation-codex.md:114-120`, `docs/process/plans/2026-05-09-hcsf-v04-implementation-codex.md:188-202`, `docs/specs/protocol-translation.md:175-197`）。
...
这些 postponed hooks 与合成计划一致：D4/D8/D9/D10/D13/D14 都推到 implementation phase，不在 P-0 重新评估（`docs/process/plans/2026-05-09-hcsf-v04-implementation-synthesis.md:166-175`）。不新增 runtime dependency 也符合 Codex lane 对 P-0/P-5 测试依赖的风险提示（`docs/process/plans/2026-05-09-hcsf-v04-implementation-codex.md:352-352`）。

## 风险与盲点

1. `feedback_chinese_comments.md` 未在仓库中定位到；我按仓库中已有 Owner 记忆记录执行中文注释约束，但无法引用该原文件本体（`docs/process/plans/2026-05-08-pasr-lite-v2-codex.md:154`, `docs/process/plans/2026-05-09-next-pivot-claude.md:228`）。
...
- `docs/process/plans/2026-05-09-hcsf-v04-implementation-synthesis.md:23-36` — P-0/P-7 phase split and Owner gates.
- `docs/process/plans/2026-05-09-hcsf-v04-implementation-synthesis.md:38-64` — 14 capability synthesis and D2 recommendation.
- `docs/process/plans/2026-05-09-hcsf-v04-implementation-synthesis.md:81-98` — test layers and failure modes.
- `docs/process/plans/2026-05-09-hcsf-v04-implementation-synthesis.md:123-175` — approved D1/D2/D3/D5/D6/D7/D11/D12 and deferred D4/D8/D9/D10/D13/D14.
- `docs/process/plans/2026-05-09-hcsf-v04-implementation-codex.md:20-68` — Codex lane capability graph/envelope/protocol_loss proposal.
- `docs/process/plans/2026-05-09-hcsf-v04-implementation-codex.md:91-120` — P-0 outputs and compatibility risk.
- `docs/process/plans/2026-05-09-hcsf-v04-implementation-codex.md:251-263` — test strategy and real smoke labeling.
- `docs/process/plans/2026-05-09-hcsf-v04-implementation-codex.md:334-352` — decision points including field names, DB, responses, data retention, smoke, native auth, dependency.
- `docs/process/plans/2026-05-09-hcsf-canonical-synthesis.md:66-123` — approved architecture: OpenAI storefront + Anthropic side-entry + capability graph + native passthrough.
...
Source files read: HUAKAI internal docs/code only: `docs/process/plans/2026-05-09-hcsf-v04-implementation-synthesis.md`; `docs/process/plans/2026-05-09-hcsf-v04-implementation-codex.md`; `docs/process/plans/2026-05-09-hcsf-canonical-synthesis.md`; `docs/research/2026-05-09-axis3-huakai-current-state.md`; `docs/research/2026-05-09-issue-mining-cross-repo.md`; `docs/specs/protocol-translation.md`; `docs/specs/streaming-forwarder.md`; `backend/internal/proto/proto.go`; `backend/internal/proto/hcsf.go`; `backend/internal/proto/anthropic_sse.go`; `backend/internal/proto/openai_sse.go`; `backend/internal/proto/gemini_sse.go`; `backend/internal/proto/bedrock_eventstream.go`; `backend/internal/proto/capability_matrix.go`; `backend/internal/proto/field_matrix.go`; `backend/internal/gateway/forwarder.go`; `backend/internal/gateway/forwarder_types.go`; `backend/internal/gateway/upstream_dispatcher.go`.
