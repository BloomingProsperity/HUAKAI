...
- 关联:[[go-live-readiness-2026-06-19]]、[[business-model-relay-saas-decision]]、`2026-07-16-three-role-single-level-tenant-model-codex.md`
...
## 三、HUAKAI 升级 delta(融合 + 给单层租户自定义域名铺路)
...
| 租户自定义域名 | nginx 每域名手动 certbot+reload | **Caddy on-demand TLS 预留**:已获授权的单层租户域名首访自动签证;Caddyfile 末尾留模板(配 ask 授权端点,默认不启用) |
...
- on-demand TLS 默认**不启用**(防任意域名触发签发),仅作单层租户自定义域名预留模板。
