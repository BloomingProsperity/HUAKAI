...
| `codex-cli.json` | ✅ `tools/fingerprint-collector/captures/openai_codex/clienthello-template.json` + `ja3-hashes.txt` + `ja4-hashes.txt` + `metadata.json` (§13 fresh capture 2026-05-26 — Owner-run pcap via Go libpcap/npcap as admin) | ✅ `codex_cli_rs 0.128.0` invoked via `codex exec --skip-git-repo-check` | ✅ `collected_at: 2026-05-26T11:36:29Z` UTC | ✅ tools/fingerprint-collector/cmd (Go libpcap/npcap admin Windows) listening on \\Device\\NPF_{B738A021-...} (sing-tun TUN) | **PASS Gate 1** — flipped WEAK → PASS by §13 full-recapture (this commit) per §12.7 mutation rule. ⚠ **Major drift detected**: TLS shape changed from 2026-05-14 template (TLS 1.3, 30 ciphers, 11 extensions including ext 43/45/51) to current (TLS 1.2, 18 ciphers, 7 extensions, no TLS 1.3 indicators). Template rewritten in this commit; old TLS 1.3 values archived in `_comment` for trail. Cross-verified via mitmproxy localhost capture from same machine — same TLS 1.2 shape — confirms not a sing-box overlay. |
| `gemini-advanced.json` | ✅ `tools/fingerprint-collector/captures/gemini_advanced/clienthello-template.json` + sibling jsonls (§13 Owner-run pcap 2026-05-26) | ✅ `gemini --version 0.42.0` (driven via `gemini --prompt`) | ✅ `collected_at: 2026-05-26T12:12:35Z` UTC | ✅ tools/fingerprint-collector/cmd (Go libpcap/npcap admin Windows) listening on \\Device\\NPF_{B738A021-...} (sing-tun TUN) | **PASS Gate 1** — flipped WEAK → PASS by §13 (this commit). ⚠ **Major model-API path drift detected** vs 2026-05-14 0.41.2 template: 0.42.0 switched to Chrome-style TLS impersonation (16 ciphers + GREASE + Chrome-specific exts like compress_certificate, application_settings, 192-byte padding). Auxiliary endpoint variant STABLE (same JA3 hash). Template rewritten with new top-level shape + updated tls_variants. |
| `kiro-cli.json` | ⚠ `/tmp/fingerprint-data/kiro-*` paths SUPERSEDED-tagged | ✅ explicit driving client | ✅ `collected_at: 2026-05-14T06:57:12Z` | ✅ explicit tool tags | **WEAK locked — permanent KnownGap** (consistent with F-2 task #20 decision; Owner confirmed 2026-05-26 no Kiro subscription → no fresh first-party capture possible). The 2026-05-14 TLS values are the only record HUAKAI will ever have for this vendor unless Owner re-acquires a Kiro account. NOT eligible for PASS via §13. |
...
| **Codex CLI** (codex-cli 0.128.0) | `tools/fingerprint-collector/captures/openai_codex/` (Owner-run admin pcap 2026-05-26T11:36) + earlier `captures/clienthello-1779787015.jsonl` (mitmproxy run, same shape) | **CLEAN PASS + drift surfaced**: 5 ClientHello samples, all identical (ja3=6a5d235e... ja4=t12d1807...). Template rewritten with new TLS 1.2 values. Earlier diagnosis "OAuth refresh probe shape" was **wrong** — this IS codex_cli_rs 0.128.0's real ClientHello (cross-verified via mitmproxy on same machine). The 18-cipher / 7-ext / TLS 1.2 shape is current ground truth. | ✅ WEAK → **PASS** | nothing — locked in. |
| **Gemini Advanced** (gemini 0.42.0) | `tools/fingerprint-collector/captures/gemini_advanced/` (Owner-run admin pcap 2026-05-26T12:12 via tools/fingerprint-collector/cmd Go libpcap; 4 samples = 1 model-API h2 + 3 auxiliary) + earlier `captures/clienthello-1779787154.jsonl` (mitmproxy run, partially misleading) | **CLEAN PASS + drift documented**: sample #1 model-API ClientHello is a NEW Chrome-style TLS impersonation shape with GREASE (10794, 35466, 23130, 39578) + Chrome-specific extensions (27=compress_certificate, 17513=application_settings, 192-byte ext 21 padding). ALPN h2+http/1.1 PRESERVED. Earlier mitmproxy diagnosis of "Gemini 0.42.0 dropped h2 ALPN" was WRONG — mitmproxy was eating the h2 ALPN during interception. Auxiliary variant unchanged from 0.41.2. | ✅ WEAK → **PASS** | nothing — locked in. Note: HUAKAI mimicry layer needs Chrome-style TLS-impersonation library (rquest etc.) to actually emit this shape; tracked as future capability slice (HUAKAI's stock reqwest cannot produce GREASE + custom extension ordering). |
| **Kiro CLI** (kiro 0.12.184) | (none) | **NO CAPTURE** — and none possible going forward: Owner confirmed 2026-05-26 they have no Kiro subscription. Consistent with existing F-2 task #20 decision "Kiro 永久 KnownGap". | 🔒 **WEAK locked — permanent KnownGap** | No re-capture path. The 2026-05-14 captured TLS values stay as-is; §13c-kiro-recapture slice **CLOSED as permanently blocked**. Any future Kiro promotion requires Owner to re-acquire a Kiro account first. |
