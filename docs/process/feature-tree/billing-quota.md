...
**Domain summary:** HUAKAI has a sophisticated billing core (Tx1 balance-hold + Tx2 atomic settlement, trust-receipt chain, voucher, subscription, payment orders) that is partially production-wired; the quota-policy enforcement engine (`internal/quota`) is fully built in isolation but not yet imported by any production path — meaning per-window/concurrency policies currently have zero runtime effect.
...
| 18 | **User-initiated refund / full order refund** | MISSING | Grep: no `/v1/users/me/refunds` or admin order-refund endpoint | Only system-triggered mismatch refunds exist. No user-facing refund request flow or admin full-order refund. |
...
| 31 | **Quota enforcement wired into gateway hot path** | MISSING | Grep: zero non-test files import `internal/quota`; `claim_gate.go` imports only `balancehold`, not `quota` | `quota.Service.Reserve()` is fully implemented but never called from `billing/claim_gate.go` or `gatewayhttp/chat_completions_dispatch.go`. Window quotas, concurrency caps, and token/request limits have zero runtime effect on live requests. |
...
| 36 | **Concurrency limiting (per-user / per-key)** | PARTIAL | `internal/quota/types.go:37` `MetricConcurrency`; `service_integration_test.go:413` full test | Concurrency slot model is fully implemented in `quota.Service`; integration-tested. Not wired to gateway dispatch path (see #31). |
| 37 | **Token-level quota enforcement (pre-request estimate)** | PARTIAL | `internal/tokencheck/estimator.go` `HeuristicEstimator`; `MetricTokensEstimated` in quota types | Token estimator exists; `tokens_estimated` metric in quota schema. Estimator is used for crosscheck/anti-fraud but quota.Service.Reserve with token metric is never called from dispatch path. |
...
| 44 | **Spend / low-balance alert / threshold notification** | MISSING | Grep: no `SpendAlert`, `LowBalance`, `balance_threshold`, alert email in payment or billing packages | No mechanism to notify users/operators when balance drops below a threshold or daily spend exceeds a limit. Only subscription expiry reminders exist. |
...
1. **Quota enforcement wired to gateway hot path** (#31) — *Critical.*  
   The entire `internal/quota` subsystem (schema, service, reservations, settlements, audit) is implemented and integration-tested but zero production code calls `quota.Service.Reserve()`. Window quotas (daily/monthly USD caps), request-count limits, concurrency caps, and per-API-key quotas have no runtime effect. Revenue protection and SLA isolation depend on this.
...
4. **Spend / low-balance alerts** (#44) — *User retention.*  
   No notification when a user's balance is about to run out. Industry standard (OpenRouter, sub2api) sends email/webhook alerts at configurable thresholds. Silent exhaustion leads to request failures without warning.
...
12. **User-initiated refund** (#18) — *Trust / dispute resolution.*  
    Only system-triggered mismatch auto-refunds exist. No user or admin can initiate a full order refund. Required for chargebacks and billing disputes.
