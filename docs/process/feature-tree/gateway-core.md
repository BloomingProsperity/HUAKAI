...
| 69 | Anti-H2 smuggling (Rust TLS sidecar) | **PRESENT** | `exploratory/rust-core-gateway/merged/crates/tls-sidecar/` (duplicate Content-Length + duplicate Host rejection per d3d1893); also go-level ContentLength validation in `privacy/middleware.go:38` | sidecar 桥已接进生产 dispatch 路径(env 可切换):`wiring.go:296` 把 `cfg.TransportSidecarSocket` 注入 factory(注入点 `wiring.go:1178`),`factory.go:179` 按 `SidecarSocketPath != ""` 选 sidecar RoundTripper,`upstream_dispatcher.go:196` 每次上游转发都经 `TransportFactory.For(...)`;仅默认 socket 空=opt-in(`config.go:239` `HUAKAI_TRANSPORT_SIDECAR_SOCKET`)且 Docker 镜像未打包 Rust 进程 |
