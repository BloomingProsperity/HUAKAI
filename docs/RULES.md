...
> 这是规则 ID 与启动门的唯一清单；完整执行细节以 [`AGENTS.md`](../AGENTS.md) 为准。最后整理：2026-07-24。
...
| A-005 | 当前目标一个计划、一个功能分支、一个 PR；不碰另一个目标，不为同步主线另开分支或 worktree，未经 Owner 同意不合主线。 |
...
| A-009 | 所有判断、实现、测试和提交以最新 `origin/main` 为基线；开始工作和提交 PR 前先 fetch 并核对当前功能分支已包含最新主线。禁止直接在 `main` 分支或主线工作树修改、提交、推送业务内容，主线只接受经验证且获 Owner 批准的 PR。 |
| A-010 | 主线文档固定分层：规则、唯一中文白皮书、源码责任索引、现行运维手册、机器合同、GitHub 操作记录。完成计划、旧审查、旧研究、重复总纲和备份文档在有效结论吸收后删除，Git 历史承担追溯。 |
| A-011 | 分支只承载活动工作；PR 合并/关闭或功能被主线吸收后，确认无独有提交并删除本地与远端旧分支。活动 PR、独有提交和 Owner 明确保留分支不得误删。 |
| A-012 | 白皮书、架构图、源码责任索引和“当前已实现”判断只从最新 `origin/main` 提取；其他工作树、未合并分支和未合并 PR 仅是待审变更，不得混入主线现状，也不得用来补齐主线能力。 |
| A-013 | 一个目标只用一个 GitHub Issue 保存当前计划、状态、决策和未完成项；PR 保存差异与验证，commit 保存补丁，CI 保存机器证据，白皮书只保存现行真相。禁止再建会话进度、交接报告或完成计划归档。 |
...
| ID | 规则 |
| --- | --- |
| P-001 | 非平凡工作更新当前唯一 GitHub Issue 中的执行计划；强制并行双计划制度已退役。 |
...
| P-005 | 当前计划是临时执行物；目标完成后，长期结论进入白皮书或现行手册，缺陷和变更证据留在 Issue/PR/commit/Release，关闭 Issue，不复制计划归档。 |
| P-006 | 不永久指定 AI 分工；交接只更新当前 Issue/PR，必须写明分支、最后可信 SHA、已完成、未完成、验证、阻塞、工作树状态和外部副作用，接手者重新核实后继续。 |
...
| I-002 | 非测试文件 600 行、目录 6000 行/20 文件为机器预算；存量超标项不做无需求专项重构，Bug 修复只做最小必要改动；新功能优先落到内聚子包，并受既有 +5% 余量约束。 |
| I-003 | 只有拆分使体量下降后才可重生成 codebudget baseline。 |
| I-004 | 删除旧链路前证明替代入口、接线、测试和运维恢复齐全。 |
| I-005 | 不回滚用户或其他目标改动；不修改当前目标之外的 worktree。 |
| I-006 | 当前主线不保留旧业务前端；新前端只以生产源码、OpenAPI、三身份权限和运行合同为设计依据，不从历史分支、构建产物或旧页面恢复实现。 |
| I-007 | 不把大文件拆分混进普通缺陷 PR；只有新能力需要、关键缺陷无法安全修复或 Owner 单独立项时，才做受判别测试保护的局部拆分。 |
...
| PR-003 | PR 提交前再次 fetch `origin/main`；若主线有增量，先无丢失地同步到当前功能分支并完成冲突核实与回归测试，禁止绕过 PR 直接修主线。 |
| PR-004 | 多个执行会话共用 GitHub 身份时，PR 必须记录写入会话与独立复核会话；同账号评论不能冒充平台可证明的独立人工审批。 |
