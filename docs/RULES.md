...
| R-003 | 支付、退款、订单、订阅、账本、身份、风控、审计、前端等专业模块必须补对应领域头部项目，三镜不是万能最佳实践。 |
...
| F-003 | 合并能力必须证明每个原用户结果、运营控制、权限、审计、失败和恢复路径仍存在。 |
...
| ID | 规则 |
| --- | --- |
| C-001 | 追完整链：入口 -> 身份 -> 规范化 -> 决策 -> 持久化 -> 外部副作用 -> 异步 -> retry/fallback -> health -> billing/quota -> audit -> DLQ/recovery -> 用户/管理员状态。 |
...
| C-005 | 新增/修复能力与 Day-2 运维一起交付：查询、筛选、重试、对账、隔离、人工解决和审计。 |
