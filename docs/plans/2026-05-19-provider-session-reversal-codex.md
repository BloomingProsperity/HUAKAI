...
| Failure modes | Existing adapters may only build requests and may not execute upstream calls, so `httptest.Server` response handling/DLQ assertions may be impossible under the local sandbox or current interface. Mitigation: use socket-free fake `RoundTripper` request capture now, add skip tests for reauth/DLQ paths with precise missing implementation notes. |
