...
- **Skip the planning step**. Every codex batch from now on writes a plan first to docs/plans/, gets Owner sign-off, then dispatches.
...
Going forward: every codex batch dispatch is preceded by a `docs/plans/<date>-<taskname>.md` artifact.
