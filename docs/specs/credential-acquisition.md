...
| openai | `codex_cli_oauth` | dedicated tenant-scoped `account-imports/codex` plan/execute | operator runtime OAuth token URL + client ID; imported endpoint/client fields cannot override it | Official token-mode `auth.json`, token objects, arrays/JSONL/adjacent JSON, or raw access-token lines normalized to an encrypted session/OAuth payload. | Implemented; forced vendor/mode, no local path reads, imported identity remains untrusted, PAT and Agent Identity are rejected for their separate contracts. |
