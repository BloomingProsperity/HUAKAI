...
13. **Package & file structure discipline (added 2026-05-22 Owner directive; REVISED 2026-06-11 — hard freeze replaced by enforced soft budget)**: Owner quote "主要是怎么杜绝?你给我们的规则写进 codex 必读的文件里". Code is organized **by responsibility** — hard rule ("规则非纪律"), governs Go packages, Go files, Rust modules alike. New feature area → new appropriately-scoped package; **never** default-dump into an existing large package.
    - **[2026-06-11 revision]** The 2026-05-22 hard freeze ("do NOT add new files to `backend/internal/{gatewayhttp,gateway,proto}`") is RETIRED — measured outcome: the freeze only blocked new files, pushing new logic into existing files and bloating them (gatewayhttp grew to a 13k+ non-test-line god package with zero subpackages). Replacement is an **enforced soft budget**: `backend/internal/codebudget` (runs inside the standard unit gate) caps every non-test Go file at ≤600 lines and every package directory at ≤6000 non-test lines / ≤20 files; pre-existing violators are grandfathered in `baseline.json` at their current size with a 5% growth allowance — exceeding the allowance fails the gate, and the fix is to split a subpackage (model: `internal/provider`, 9 core files + 13 subpackages), never to regenerate the baseline upward. Baseline regeneration (`HUAKAI_REWRITE_CODE_BUDGET_BASELINE=1`) is legitimate only after a deliberate split/refactor that REDUCES size.
    - New files in the formerly-frozen packages are allowed again when the budget gate stays green, but new functionality still prefers a new subpackage (e.g. `proto/<family>/`); adding to an over-budget package burns its limited growth allowance.
    - Any plan/spec creating new files MUST name each file's target package and confirm the budget gate stays green. Canonical rule + enforcement detail: `AGENTS.md` §"Package & File Structure Discipline". Pre-commit review (#8) + slice cross-review (#7) MUST flag a package pushed past budget or mixed responsibilities as HIGH and block landing.
