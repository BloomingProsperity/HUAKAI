...
8. **Per-commit Codex review (added 2026-04-29 by Owner directive; termination criteria added 2026-05-23 per Codex independent consult, Owner approved with "同意")**: BEFORE every commit, run `codex exec review --uncommitted --full-auto` and address HIGH findings. Optionally run `codex exec review --commit <SHA> --full-auto` after commit for retro-check. See `AGENTS.md` §"Per-Commit Cross-Review Discipline" for full workflow. This applies to doc-only commits as well — the discipline catches stale cross-references and unintended scope creep, not just code defects.

   **Termination criteria (added 2026-05-23)**: Review iteration must converge, not be infinite. **P1 is reserved for**: (a) source not locatable; (b) major parity claim without evidence; (c) implementation-pollution risk from non-MIT sources; (d) real safety / security / clean-room violations. **Citation format, path-precision details, absence-claim form, doc-style nits → demote to P2/P3, do NOT block commit**. **After 2 review rounds, if remaining findings are only P2/P3 citation form / path detail / absence form**, freeze the artifact, commit it, and move surfaced items to `docs/process/citation-cleanup-backlog.md` for later batched cleanup. Real safety / clean-room violations (verbatim copy, AGPL/LGPL contamination, broken architecture) must always be fixed regardless of round count. Owner override required to extend beyond 2 rounds purely for form polish.
...
11. **Clean-room enforcement (added 2026-04-30; relaxed 2026-05-23 to risk-stratified version per Codex independent consult + Owner approval "同意")**: lane-isolation requirement depends on **what is being handled** — not blanket required. Owner quote ratifying relaxation: "同意" (2026-05-23 after Codex consult).

   **Risk stratification** (determines whether specifier/implementer split is mandatory):
   - **L0 — 行为 parity** (protocol behavior, error scenarios, API compatibility, operational workflow, public spec compliance, "X 怎么处理 429" 类问题): **same session allowed** for reading non-MIT source + writing HUAKAI implementation. No lane split required.
   - **L1 — algorithm/structure mimicry** (upstream exposes a distinctive algorithm / internal data structure / schema / test design / control flow **AND** HUAKAI implementation will be structurally close to it): **strict lane split required** — specifier session reads source and produces paraphrased behavior summary; separate implementer session reads only that spec. L1 specifier prompt template in AGENTS.md §Clean-Room Codex Prompt Template.
   - **L2 — verbatim copying** (function names / struct fields / comments / code blocks of any size copied from non-MIT source into HUAKAI): **永久禁止**, enforced by Codex per-commit review + mechanical similarity scan.
   - **L3 — vendor non-MIT** (LGPL/AGPL source code placed inside HUAKAI repository under any path): **永久禁止**, enforced by `cargo-deny` license policy + pre-commit grep for known reference project markers.
   - **L4 — AGPL service contamination** (incorporating AGPL code into HUAKAI's networked service while keeping MIT commercial license): **永久禁止**, design-time architectural decision.

   **Always-on hard prohibitions** (regardless of risk layer):
   - Never copy function names / struct fields / comments verbatim from non-MIT source.
   - Never copy raw code blocks of any size.
   - Never do line-by-line algorithmic translation.
   - Always paraphrase with different sentence structure than upstream code ordering.

   **Citation discipline (relaxed)**: file:line citations remain evidence in prose.
   - **Plan/spec metadata**: records `repo + full SHA + fetch date` once.
   - **In-prose citations**: may use short form `path:line` or source-map ID. Full `<owner>/<repo>@<sha>:file:line` per claim is **NOT a P1 blocker** (see §8 termination criteria).
   - **Absence claims** (e.g. "X does not do Y"): may be supported by scoped search log (`grep command + range + commit SHA + limitations noted`) instead of impossible line citations.

   **L1 closing requirement**: specifier session output ends with "Source files read: <list>" + Lane + Agent + UTC timestamp. L0 sessions don't need this.

   **Mechanical enforcement** (mandatory infrastructure, see `docs/process/clean-room/mechanical-enforcement.md` for setup): (1) pre-commit hook scans non-permissive license markers (`AGPL`, `LGPL`, `GNU Affero`, `GNU Lesser`) and known reference project names (`sub2api`, `clewdr`); (2) `cargo-deny` blocks AGPL/GPL/LGPL deps from default builds; (3) HUAKAI source ⟷ `~/refs/sub2api/` `~/refs/clewdr/` token-level similarity scan flags long matches; (4) specifier-derived distinctive identifiers enter denylist for implementation grep.

   **Commit attestation** (every commit body ends with): `Clean-room-attestation: original HUAKAI implementation; no copied source/comments/tests/schemas from non-permissive references.`
12. **Source-must-read rule (added 2026-05-09 Owner directive — STRENGTHENS #11 from "if-read" to "must-read")**: Owner quote "去读源码！讲规则里面改下必须读源码". Earlier rule #11 only constrained the act of reading; this rule adds that reading is **mandatory** when making any of the following claim types — Claude self-execution OR Codex dispatch alike:
    ...
    Required citation form per claim: `<repo>@<commit-sha>:<file>:<line-range>` (full form recorded in **metadata** once; **in-prose** may use short form `path:line` or source-map ID per #11 relaxation 2026-05-23; full per-claim form is **NOT a P1 blocker** after 2 review rounds per #8 termination criteria 2026-05-23). Memory recall, training-time familiarity, second-hand summaries, and README-only reads are **not** sufficient evidence. Where docs and source disagree, source is authoritative — public docs may misrepresent deferred features or undocumented edge cases.
